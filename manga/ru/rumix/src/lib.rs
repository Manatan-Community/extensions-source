...
const SOURCE: RuMix = RuMix;
const DEFAULT_BASE_URL: &str = "https://rumix.me";
const NAME: &str = "RuMIX";
...
const DETAILS_FIXTURE: &str = r#"<div class="cr-hero-names__main">Sample</div><div class="cr-info-details"><div><span class="cr-info-details-item__title">Выпуск</span><span class="cr-info-details-item__status">продолжается</span></div></div><div class="cr-main-person-item"><span class="cr-main-person-item__role">Автор</span><span class="cr-main-person-item__name">Author</span></div><a href="/list/category/el_6421">Манга</a><a href="/list/limitation/el_3968">R</a><div class="cr-tags"><span class="cr-tags__item"><span>боевик</span></span></div><div class="cr-description__content">Description</div><img class="cr-hero-poster__img" src="/cover.jpg"><table><tr class="item-row"><td class="item-title" data-num="10"></td><td><a class="chapter-link" href="/sample/vol1/1" title="Team">Глава 1</a></td><td class="d-none">01.01.24</td></tr></table><script>rm_h.readerInit('1','//img.example/',"/p.jpg");</script>"#;

struct RuMix;

impl MangaSource for RuMix {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        Ok(parse_listing(
            &fetch_document(
                &request,
                &format!(
                    "{base}/list?sortType={sort}&offset={}",
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.starts_with(&base) || query.starts_with("slug:") {
            ...
            return Ok(Paged {
                entries: vec![parse_details(
                    &fetch_document(&request, &absolute_url(&base, &key), DETAILS_FIXTURE),
                    Some(key),
                    &base,
                )],
                has_next_page: false,
        ...
        Ok(parse_listing(
            &fetch_document(&request, &target, LIST_FIXTURE),
            &base,
        ))
    }
        ...
        if input.starts_with(&base) {
            ...
            return Ok(Some(UrlResolveResult {
                item: Some(parse_details(
                    &fetch_document(&request, input, DETAILS_FIXTURE),
                    Some(key),
                    &base,
                )),
...
fn base_url(request: &Value) -> String {
    preference(request, "domain", DEFAULT_BASE_URL)
        .trim_end_matches('/')
        .to_string()
}
        ...
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                cover: html::attr_after(chunk, "img", "data-original")
                    ...
                    .map(|v| absolute_url(base, &v.replace("_p.", "."))),
...
fn parse_details(body: &str, key: Option<String>, base: &str) -> CatalogItem {
    if body.contains("expandable") || body.contains("elem_category") {
        return parse_rumix_details(body, key, base);
    }
    ...
    tags.extend(
        body.split("<a")
            .skip(1)
            .filter(|c| c.contains("/list/category/") || c.contains("/list/limitation/"))
            .filter_map(|c| {
                html::text_between(c, ">", "</a>").map(|v| normalize_age(&html::strip_tags(&v)))
            }),
    );
    tags.extend(
        body.split("cr-tags__item").skip(1).filter_map(|c| {
            html::text_between(c, "<span", "</span>").map(|v| html::strip_tags(&v))
        }),
    );
    CatalogItem {
        key: key.clone(),
        title: html::text_between(body, "cr-hero-names__main", "</")
            .or_else(|| html::attr_after(body, "itemprop=name", "content"))
            .map(|v| html::strip_tags(&v))
            .filter(|v| !v.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| NAME.into())),
        cover: html::attr_after(body, "cr-hero-poster__img", "src")
            .or_else(|| html::attr_after(body, "cr-hero-poster__img", "data-original"))
            .or_else(|| html::attr_after(body, "cr-hero-overlay__bg", "data-bg"))
            .map(|v| absolute_url(base, &v)),
...
fn parse_rumix_details(body: &str, key: Option<String>, base: &str) -> CatalogItem {
    let key = key.unwrap_or_else(|| "/sample".into());
    let info = body.split("expandable").nth(1).unwrap_or(body);
    let category = class_text(info, "elem_category").unwrap_or_else(|| "manga".into());
    let age = class_text(info, "elem_limitation")
        .map(|v| normalize_age(&v))
        .unwrap_or_default();
    let mut tags = vec![category, age];
    tags.extend(
        body.split("<a")
            .skip(1)
            .filter(|c| c.contains("/list/genre/") || c.contains("/list/tag/"))
            .filter_map(|c| html::text_between(c, ">", "</a>").map(|v| html::strip_tags(&v))),
    );
    let rating = html::attr_after(info, "rating-block", "data-score")
        .and_then(|v| v.replace(',', ".").parse::<f32>().ok())
        .map(|v| v * 2.0);
    let rating_votes =
        html::attr_after(info, "ratingCount", "content").unwrap_or_else(|| "0".into());
    let rating_summary = rating
        .map(|value| {
            format!(
                "{} {value} (голосов: {rating_votes})\n",
                rating_to_stars(value)
            )
        })
        .unwrap_or_default();
    let alt = class_text(info, "another-names")
        .filter(|v| !v.is_empty())
        .map(|v| format!("Альтернативные названия:\n{v}\n\n"))
        .unwrap_or_default();
    let desc = html::text_between(body, "tab-description", "</div>")
        .map(|v| html::strip_tags(&v))
        .unwrap_or_default();
    let badges = info
        .split("badge")
        .skip(1)
        .filter_map(|c| {
            html::text_between(c, ">", "</").map(|v| html::strip_tags(&v).to_lowercase())
        })
        .collect::<Vec<_>>()
        .join(" ");
    let lower = body.to_lowercase();
    let restricted = (lower.contains("запрещен") && lower.contains("копирайт"))
        || (lower.contains("территории рф") && lower.contains("запрещен"));
    CatalogItem {
        key: key.clone(),
        title: class_text(body, "names")
            .or_else(|| class_text(body, "name"))
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| NAME.into())),
        cover: html::attr_after(info, "<img", "data-full")
            .or_else(|| html::attr_after(info, "<img", "data-original"))
            .or_else(|| html::attr_after(info, "<img", "src"))
            .map(|v| absolute_url(base, &v)),
        authors: class_text(info, "elem_author")
            .or_else(|| class_text(info, "elem_screenwriter"))
            .into_iter()
            .collect(),
        artists: class_text(info, "elem_illustrator").into_iter().collect(),
        tags: clean_tags(tags),
        description: Some(format!("{rating_summary}{alt}{desc}")).filter(|v| !v.trim().is_empty()),
        status: if restricted && !body.contains("div class=\"chapters") {
            ItemStatus::Unknown
        } else if info.contains("<b>Сингл") || badges.contains("заверш") {
            ItemStatus::Completed
        } else if badges.contains("продолж") || badges.contains("начат") {
            ItemStatus::Ongoing
        } else if badges.contains("приост") || badges.contains("заморож") {
            ItemStatus::Hiatus
        } else {
            ItemStatus::Unknown
        },
...
fn parse_chapters(body: &str, base: &str) -> Vec<MangaChapter> {
    let suffix = user_hash(body)
        .map(|hash| format!("?d={hash}&mtr=true"))
        .unwrap_or_else(|| "?mtr=true".into());
    body.split("item-row")
        .skip(1)
        ...
        .filter_map(|chunk| {
            ...
            let key = normalize_key(base, &href);
            let number = html::attr_after(chunk, "item-title", "data-num")
                .and_then(|v| v.parse::<f32>().ok())
                .map(|v| v / 10.0);
            Some(MangaChapter {
                key: format!("{key}{suffix}"),
                title: html::text_between(chunk, "chapter-link", "</a>")
                    .map(|v| html::strip_tags(&v).replace(" новое", ""))
                    .filter(|v| !v.is_empty()),
                scanlators: html::attr_after(chunk, "chapter-link", "title")
                    .map(|v| v.replace("(Переводчик),", "&").replace(" (Переводчик)", ""))
                    .filter(|v| !v.is_empty())
                    .into_iter()
                    .collect(),
                chapter_number: number,
                date_uploaded: html::text_between(chunk, "d-none", "</")
                    .map(|v| html::strip_tags(&v))
                    .and_then(|v| dates::parse_fixture_date(&v)),
                url: Some(absolute_url(base, &key)),
...
fn parse_pages(body: &str, base: &str) -> Vec<MangaPage> {
    ...
    let script = body
        ...
        .unwrap_or_default();
    let fields = quoted_fields(script);
    fields
        .chunks(3)
        .enumerate()
...
fn description(body: &str) -> Option<String> {
    let rating =
        html::text_between(body, "cr-hero-rating__value", "</").map(|v| html::strip_tags(&v));
    let votes =
        html::text_between(body, "cr-hero-rating__text", "</").map(|v| html::strip_tags(&v));
    let prefix = rating
        .and_then(|r| {
            r.parse::<f32>().ok().map(|value| {
                format!(
                    "{} {value}{}\n",
                    rating_to_stars(value),
                    votes
                        .map(|v| format!(
                            " (голосов: {})",
                            v.chars().filter(char::is_ascii_digit).collect::<String>()
                        ))
                        .unwrap_or_default()
                )
            })
        ...
        .unwrap_or_default();
    let text = html::text_between(body, "cr-description__content", "</")
        .map(|v| html::strip_tags(&v))
        .unwrap_or_default();
...
fn detail_value(body: &str, label: &str) -> Option<String> {
    body.split("cr-info-details-item__title")
        .skip(1)
        .find(|c| c.contains(label))
        .and_then(|c| html::text_between(c, "cr-info-details-item__status", "</"))
        .map(|v| html::strip_tags(&v).to_lowercase())
}

fn people(body: &str, role: &str) -> Vec<String> {
    body.split("cr-main-person-item")
        .skip(1)
        .filter(|c| c.to_lowercase().contains(role))
        .filter_map(|c| {
            html::text_between(c, "cr-main-person-item__name", "</").map(|v| html::strip_tags(&v))
        })
        .collect()
}

fn parse_status(release: &str, translation: &str) -> ItemStatus {
    if release.contains("заверш") && translation.contains("заверш") {
        ItemStatus::Completed
    } else if release.contains("заверш") {
        ItemStatus::Completed
    } else if release.contains("приост") || release.contains("заморож") {
        ItemStatus::Hiatus
    } else if release.contains("продолж") || release.contains("начат") {
        ItemStatus::Ongoing
    } else {
        ItemStatus::Unknown
    }
...
fn normalize_age(raw: &str) -> String {
    match raw.trim() {
        "NC-17" | "R18+" => "18+".into(),
        "R" | "G" | "PG" => "16+".into(),
        "PG-13" => "12+".into(),
...
fn rating_to_stars(value: f32) -> &'static str {
    if value > 9.5 {
        "★★★★★"
    } else if value > 8.5 {
        "★★★★✬"
...
fn user_hash(body: &str) -> Option<String> {
    body.split("user_hash")
        .nth(1)
        .and_then(|tail| quoted_fields(tail).into_iter().next())
}
...
fn filter_id<'a>(filters: Option<&'a Value>, id: &str) -> Option<&'a str> {
    filters.and_then(|f| f.get(id)).and_then(|v| {
        v.as_str()
