...
const SOURCE: MangaChan = MangaChan;
const BASE_URL: &str = "https://im.manga-chan.me";

struct MangaChan;

impl MangaSource for MangaChan {
...
fn catalog_url(page: u64, filters: &Value) -> String {
    ...
    let tag_path = genres
        .iter()
        .map(String::as_str)
        .chain(excluded.iter().map(|v| v.as_str()))
        .collect::<Vec<_>>();
    ...
    if tag_path.is_empty() {
        let path = match sort {
            "dateasc" => "manga/new&n=dateasc".to_string(),
            "abcasc" => "catalog".to_string(),
            "chdesc" => "sortch".to_string(),
            _ => "mostfavorites".to_string(),
        };
        if status.is_empty() {
        ...
        let order = if sort.is_empty() {
            ...
            format!("?n={sort}&offset={offset}")
        };
        if status.is_empty() {
            format!("{BASE_URL}/tags/{}{order}", tags.join("+"))
        } else {
            format!(
                "{BASE_URL}/tags/{}?offset={offset}&status={status}",
                tags.join("+")
            )
        }
    }
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "<title", "</title>")
            ...
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "MangaChan".into())),
...
const DETAILS_FIXTURE: &str = r#"<title>Sample » MangaChan</title><img id="cover" src="/cover.jpg"><div id="description">Description</div><table class="table_cha"><tr><td></td></tr><tr><td><a href="/sample/1">Глава 1</a><div class="date">2024-01-01</div></td></tr></table>"#;
