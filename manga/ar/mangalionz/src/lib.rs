...
use manatan_shared::{manga, manga::MadaraConfig, sdk::SearchRequest, sdk::http::HttpClient, url};
use serde_json::Value;

const SOURCE: MangaLionz = MangaLionz;

struct MangaLionz;

impl MangaSource for MangaLionz {
...
fn config(_request: &Value) -> MadaraConfig {
    MadaraConfig {
        base_url: "https://manga-lionz.org",
        lang: "ar",
        content_rating: "safe",
        manga_path: "manga",
...
fn search_ajax(config: &MadaraConfig, query: &str) -> Paged<CatalogItem> {
    let body = HttpClient::browser()
        ...
        .form(&[("action", "wp-manga-search-manga"), ("title", query)])
        .send_text()
    ...
    let entries = root
        ...
        .flatten()
        .filter_map(|entry| {
            let item_url = entry.get("url").and_then(Value::as_str)?;
            let key = config.normalize_manga_key(item_url);
            Some(CatalogItem {
                key: key.clone(),
                title: entry
                    .get("title")
...
const SEARCH_FIXTURE: &str = r#"{"success":true,"data":[{"url":"https://manga-lionz.org/manga/sample/","title":"Sample Manga"}]}"#;
...
mod tests {
    ...
    fn parses_madara_source() {
        ...
        let search = SOURCE.search(json!({"query":"sample"})).unwrap();
        assert_eq!(search.entries[0].key, "/manga/sample");
