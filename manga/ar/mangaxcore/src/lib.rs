...
const SOURCE: MangaxCore = MangaxCore;

struct MangaxCore;

impl MangaSource for MangaxCore {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let config = config();
        let key =
            manga::request_key(&request, "manga").unwrap_or_else(|| "/works/sample".to_string());
        let body =
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/works/sample/chapter-1".to_string());
        let body =
...
<div class="page-item-detail"><h3 class="post-title"><a href="/works/sample/">Sample Manga</a></h3><img src="/cover.jpg"></div>
...
<ul class="main version-chap"><li class="wp-manga-chapter"><a href="/works/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">01/01/2024</span></li></ul>
"#;
...
mod tests {
    ...
    fn parses_madara_source() {
        ...
        let chapters = SOURCE.chapters(json!({"manga":"/works/sample"})).unwrap();
