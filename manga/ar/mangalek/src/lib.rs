...
const SOURCE: Mangalek = Mangalek;

struct Mangalek;

impl MangaSource for Mangalek {
...
fn config(request: &Value) -> MadaraConfig {
    ...
    let base_url = preferences
        .and_then(|prefs| prefs.get("customBaseUrl"))
        .and_then(Value::as_str)
...
const SEARCH_FIXTURE: &str = r#"{"success":true,"data":[{"url":"https://lekmanga.net/manga/sample/","title":"Sample Manga"}]}"#;
