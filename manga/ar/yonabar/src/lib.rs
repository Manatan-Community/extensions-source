...
const SOURCE: YonaBar = YonaBar;

struct YonaBar;

impl MangaSource for YonaBar {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let config = config();
        let key =
            manga::request_key(&request, "manga").unwrap_or_else(|| "/yaoi/sample".to_string());
        let body =
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/yaoi/sample/chapter-1".to_string());
        let body =
...
fn rewrite_medium_page(mut page: MangaPage) -> MangaPage {
    if let PageContent::Url { url, .. } = &mut page.content {
        *url = url.replace("medium1", "medium1x");
    }
    page
}
...
<div class="page-item-detail"><h3 class="post-title"><a href="/yaoi/sample/">Sample Manga</a></h3><img src="/cover.jpg"></div>
...
<ul class="main version-chap"><li class="wp-manga-chapter"><a href="/yaoi/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">01/01/2024</span></li></ul>
"#;
const PAGES_FIXTURE: &str = r#"<div class="reading-content"><img class="wp-manga-chapter-img" src="/medium1/page1.jpg"><img class="wp-manga-chapter-img" src="/medium1/page2.jpg"></div>"#;
...
mod tests {
    ...
    fn parses_madara_source() {
        ...
        let chapters = SOURCE.chapters(json!({"manga":"/yaoi/sample"})).unwrap();
        assert!(!chapters.is_empty());

        let pages = SOURCE
            .pages(json!({"chapter":"/yaoi/sample/chapter-1"}))
            .unwrap();
        assert!(format!("{:?}", pages[0].content).contains("medium1x"));
    }
}
