...
const SOURCE: YuriMoonSub = YuriMoonSub;
const BASE_URL: &str = "https://yurimoonsub.blogspot.com";
const MANGA_CATEGORY: &str = "Series";
const CHAPTER_CATEGORY: &str = "Chapter";

struct YuriMoonSub;

impl MangaSource for YuriMoonSub {
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        let label = html::attr_after(&body, "manga-widget", "data-label")
            .map(|label| clean_chapter_label(&label))
...
fn clean_chapter_label(input: &str) -> String {
    input
        .chars()
...
<div class="manga-widget" data-label="Sample Chapters فصول"></div>
"#;

const FEED_FIXTURE: &str = r#"{
  "feed": {"entry": [{"title":{"$t":"Sample Manga"},"category":[{"term":"Series"}],"link":[{"rel":"alternate","href":"https://yurimoonsub.blogspot.com/2024/01/sample.html"}],"media$thumbnail":{"url":"https://img/cover.jpg"}}]}
}"#;

const CHAPTER_FEED_FIXTURE: &str = r#"{
  "feed": {"entry": [{"title":{"$t":"Chapter 1"},"published":{"$t":"2024-01-01T00:00:00.000Z"},"category":[{"term":"Chapter"}],"link":[{"rel":"alternate","href":"https://yurimoonsub.blogspot.com/2024/01/chapter-1.html"}]}]}
}"#;
...
mod tests {
    ...
    fn parses_zeist_source() {
        ...
        assert_eq!(
            clean_chapter_label("Sample Chapters فصول"),
            "Sample Chapters"
        );
