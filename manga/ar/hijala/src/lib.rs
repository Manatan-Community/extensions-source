...
const SOURCE: Hijala = Hijala;
const BASE_URL: &str = "https://hijala.com";

struct Hijala;

impl MangaSource for Hijala {
...
fn parse_pages_from_html(body: &str) -> Vec<MangaPage> {
    ...
    if !body.contains("chapter-pages-js-before") {
        return normal_pages;
    }
    ...
    normal_pages
        ...
        .filter_map(|(index, pair)| {
            ...
            let left_url = page_url(left)?;
            let right_url = page_url(right)?;
            let key = format!("paired:{left_url}|{right_url}");
            Some(MangaPage {
                ...
                headers: manga::image_headers(BASE_URL),
                description: Some(format!("Page {}", index + 1)),
                extra: paired_image_extra(&left_url, &right_url),
                ..MangaPage::default()
            })
        })
        .collect()
...
fn paired_image_extra(left_url: &str, right_url: &str) -> BTreeMap<String, Value> {
    BTreeMap::from([
        (
            "imageProcessing".to_string(),
            Value::String("join-horizontal".to_string()),
        ),
        ("leftImage".to_string(), Value::String(left_url.to_string())),
        (
            "rightImage".to_string(),
            Value::String(right_url.to_string()),
        ),
    ])
}

fn page_url(page: &MangaPage) -> Option<String> {
    match &page.content {
        PageContent::Url { url, .. } => Some(url.clone()),
        PageContent::Lazy { url: Some(url), .. } => Some(url.clone()),
        _ => None,
    }
}
...
const SECURE_API_FIXTURE: &str = r##"{"success":true,"data":{"status":"unlocked","content":"<div id='chapter-pages-js-before'></div><img src='/page1-left.jpg'><img data-src='/page1-right.jpg'>"} }"##;
