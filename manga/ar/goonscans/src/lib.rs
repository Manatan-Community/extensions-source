...
const SOURCE: GoonScans = GoonScans;
const BASE_URL: &str = "https://goonscans.org";

struct GoonScans;

impl MangaSource for GoonScans {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = format!("{BASE_URL}/title/?page={page}&order={order}");
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let key =
            manga::request_key(&request, "manga").unwrap_or_else(|| "/title/sample/".to_string());
        let body = fetch_document_or_fixture(&url::join_url(BASE_URL, &key), DETAILS_FIXTURE);
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/title/sample/chapter-1/".to_string());
...
<div class="listupd"><div class="manga-card-v"><a href="/title/sample/" title="Sample Manga"><img data-src="/cover.jpg"></a><div class="bigor"><div class="tt">Sample Manga</div></div></div></div>
...
<div id="chapters-list-container"><div class="ch-item"><a href="/title/sample/chapter-1/"><span class="chap-num">Chapter 1</span><span class="chap-date">2024/01/01</span></a></div></div></div>
"#;
...
mod tests {
    ...
    fn parses_listing_details_and_chapters() {
        ...
        let details = parse_details(DETAILS_FIXTURE, Some("/title/sample/".into()));
        ...
        assert_eq!(chapters[0].key, "/title/sample/chapter-1/");
    }
