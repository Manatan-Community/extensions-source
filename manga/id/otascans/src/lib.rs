...
const SOURCE: OtaScans = OtaScans;
const BASE_URL: &str = "https://yurilabs.my.id";

struct OtaScans;

impl MangaSource for OtaScans {
...
fn fetch_ajax_chapters(manga_key: &str, config: &MadaraConfig) -> Vec<MangaChapter> {
    let manga_url = config
        ...
        .trim_end_matches('/')
        .to_string();
    let endpoint = format!("{manga_url}/ajax/chapters/");
    let mut chapters = Vec::new();
    let first = fetch_ajax_page(&endpoint, 1, config);
    if first.trim().is_empty() || !first.contains("wp-manga-chapter") {
        return Vec::new();
    }
    chapters.extend(manga::Madara::parse_chapters(&first, manga_key, config));
    for page in 2..=last_ajax_page(&first) {
        let body = fetch_ajax_page(&endpoint, page, config);
        if !body.contains("wp-manga-chapter") {
            continue;
        }
        chapters.extend(manga::Madara::parse_chapters(&body, manga_key, config));
    }
    chapters.into_iter().fold(Vec::new(), push_unique_chapter)
}

fn fetch_ajax_page(endpoint: &str, page: u64, config: &MadaraConfig) -> String {
    manga::Madara::browser_client(config)
        .post(format!("{endpoint}?t={page}"))
        .xhr()
        .send_text()
        .unwrap_or_default()
}

fn last_ajax_page(body: &str) -> u64 {
    body.split("data-page")
        .skip(1)
        .filter_map(|chunk| html::attr(chunk, "data-page"))
...
<div class="manga__item"><h3 class="post-title"><a href="/series/sample/">Sample Ota Scans</a></h3><img src="/cover.jpg"></div>
<div class="navigation-ajax"></div>
"#;
const DETAILS_FIXTURE: &str = r#"
<h1 class="post-title">Sample Ota Scans</h1><div class="summary_image"><img src="/cover.jpg"></div>
<div class="description-summary">Sample description.</div>
<ul><li class="wp-manga-chapter"><a href="/series/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">1 January 2024</span></li></ul>
"#;
...
mod tests {
    ...
    fn parses_madara_fixture() {
        ...
        assert_eq!(last_ajax_page(r#"<a data-page="3">3</a>"#), 3);
    }
}
