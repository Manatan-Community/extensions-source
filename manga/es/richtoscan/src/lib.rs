...
const SOURCE: RichtoScan = RichtoScan;
const BASE_URL: &str = "https://r1.richtoon.top";

struct RichtoScan;

impl MangaSource for RichtoScan {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = manga::Madara::fetch_document_or_fixture(
            &config,
            &filtered_search_url(&config, page, query, request.get("filters")),
            LIST_FIXTURE,
        );
        Ok(Paged {
            entries: manga::Madara::parse_listing(&body, &config),
