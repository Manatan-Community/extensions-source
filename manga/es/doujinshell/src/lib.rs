...
const SOURCE: DoujinsHell = DoujinsHell;
const BASE_URL: &str = "https://doujinshell.net";

struct DoujinsHell;

impl MangaSource for DoujinsHell {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let config = config();
        let key =
            manga::request_key(&request, "manga").unwrap_or_else(|| "/doujin/sample".to_string());
        let body =
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        if chapters.len() == 1 {
            chapters[0].title = Some("Capitulo".to_string());
        }
        Ok(chapters)
    }
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/doujin/sample/chapter-1".to_string());
        let body =
            manga::Madara::fetch_document_or_fixture(&config, &config.absolute_url(&key), PAGES_FIXTURE);
        let pages = parse_pages(&body, &config);
        if pages.is_empty() && body.contains("<iframe") && body.contains("reading-content") {
            return Err(ExtensionError {
                message: "No se admiten videos".to_string(),
            });
        }
        Ok(pages)
    }
...
fn parse_pages(body: &str, config: &MadaraConfig) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| !chunk.contains("aligncenter"))
        .filter_map(|chunk| {
...
const LIST_FIXTURE: &str = r#"<div class="page-item-detail"><div class="post-title"><h3><a href="https://doujinshell.net/doujin/sample">Sample</a></h3></div><img src="https://doujinshell.net/sample.jpg"></div><div class="nav-previous"></div>"#;
const DETAILS_FIXTURE: &str = r#"<div class="post-title"><h1>Sample</h1></div><div class="summary_image"><img src="https://doujinshell.net/sample.jpg"></div><div class="listing-chapters_wrap"><li class="wp-manga-chapter"><a href="https://doujinshell.net/doujin/sample/chapter-1">Chapter 1</a></li></div>"#;
const PAGES_FIXTURE: &str = r#"<div class="reading-content"><img class="wp-manga-chapter-img" src="https://doujinshell.net/page-1.jpg"><img class="aligncenter" src="https://doujinshell.net/next.jpg"></div>"#;
