...
const SOURCE: KnightNoScanlation = KnightNoScanlation;
const BASE_URL: &str = "https://lectorknight.com";

struct KnightNoScanlation;

impl MangaSource for KnightNoScanlation {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let config = config();
        let key = manga::request_key(&request, "manga").unwrap_or_else(|| "/sr/sample".into());
        let body =
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/sr/sample/chapter-1".into());
        let body =
...
<div class="manga__item post-title"><a href="/sr/sample/">Sample Manga</a><img src="/cover.jpg"></div>
...
<ul><li class="wp-manga-chapter"><a href="/sr/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">2024-01-01</span></li></ul>
"#;
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(
            manga::Madara::parse_chapters(DETAILS_FIXTURE, "/sr/sample", &config).len(),
            1
        );
    }
}
