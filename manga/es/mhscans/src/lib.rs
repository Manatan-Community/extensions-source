...
const SOURCE: MHScans = MHScans;
const BASE_URL: &str = "https://mhscans.com";

struct MHScans;

impl MangaSource for MHScans {
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        Ok(manga::Madara::parse_chapters(&body, &key, &config)
            .into_iter()
            .filter(|chapter| !remove_premium(&request) || !chapter.is_locked)
            .collect())
    }
...
fn remove_premium(request: &Value) -> bool {
    request
        .get("preferences")
        .and_then(|preferences| preferences.get("removePremiumChapters"))
    ...
    body.split("<img")
        ...
        .filter(|chunk| {
            chunk.contains("wp-manga-chapter-img")
                ...
                || chunk.contains("rk-img")
                || chunk.contains("rk-page-wrap")
                || chunk.contains("blocks-gallery-item")
                || chunk.contains("text-left")
        })
        .filter_map(|chunk| {
            manatan_shared::html::attr(chunk, "data-src")
                .or_else(|| manatan_shared::html::attr(chunk, "data-lazy-src"))
                .or_else(|| manatan_shared::html::attr(chunk, "data-cfsrc"))
                .or_else(|| manatan_shared::html::attr(chunk, "src"))
        })
...
<div class="page-item-detail"><h3 class="post-title"><a href="/series/sample/">Sample Manga</a></h3><img src="/cover.jpg"></div>
...
<ul><li class="wp-manga-chapter"><a href="/series/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">2024-01-01</span></li><li class="wp-manga-chapter premium"><a href="/series/sample/chapter-2/">Chapter 2</a></li></ul>
"#;
