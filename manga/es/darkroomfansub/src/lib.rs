...
const SOURCE: DarkRoomFansub = DarkRoomFansub;
const BASE_URL: &str = "https://lector-darkroomfansub.blogspot.com";
const NAME: &str = "Dark Room Fansub";
const LANG: &str = "es";
...
struct DarkRoomFansub;

impl MangaSource for DarkRoomFansub {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_feed_listing(LIST_FIXTURE, 1));
        }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let page = request.get("page").and_then(Value::as_u64).unwrap_or(1);
        Ok(parse_feed_listing(
            &fetch_text(&feed_url(page, Some(query), Vec::new()), LIST_FIXTURE),
            page,
        ))
    }
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        let chapter_feed = chapter_feed_url(&body);
        Ok(parse_chapter_feed(
            &fetch_text(&chapter_feed, CHAPTERS_FIXTURE),
            &key,
        ))
    }
...
const LIST_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Sample"},"category":[{"term":"Series"}],"link":[{"rel":"alternate","href":"https://lector-darkroomfansub.blogspot.com/p/sample.html"}],"media$thumbnail":{"url":"https://1.bp.blogspot.com/s72-c/sample.jpg"}}]}}"#;
const DETAILS_FIXTURE: &str = r#"<main id="main"><h1>Sample</h1><img src="/cover.jpg"><div id="synopsis">Summary</div><div id="extra-info"><dl><dt>Autor</dt><dd>Author</dd><dt>Estado</dt><dd>En curso</dd></dl></div><a rel="tag">Drama</a><div id="clwd"><script>clwd.run('sample')</script></div></main>"#;
const CHAPTERS_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Capitulo 1"},"published":{"$t":"2024-01-01T00:00:00.000Z"},"category":[{"term":"Chapter"}],"link":[{"rel":"alternate","href":"https://lector-darkroomfansub.blogspot.com/2024/01/sample.html"}]}]}}"#;
const PAGES_FIXTURE: &str = r#"<article id="reader"><div class="separator"><img src="/page1.jpg"></div><div class="separator"><img src="/page2.jpg"></div></article>"#;
