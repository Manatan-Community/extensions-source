...
const SOURCE: PinkGardenTranslation = PinkGardenTranslation;
const BASE_URL: &str = "https://capibaratraductor.com/pinkgardentranslation";
const API_BASE_URL: &str = "https://capibaratraductor.com";
const ORGANIZATION: &str = "pinkgardentranslation";
const NAME: &str = "PinkGarden Translation";
const LANG: &str = "es";
const CONTENT_RATING: &str = "safe";
const PAGE_LIMIT: u64 = 36;

struct PinkGardenTranslation;

impl MangaSource for PinkGardenTranslation {
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        let key = manga::request_key(&request, "manga").unwrap_or_else(|| "sample".to_string());
        Ok(parse_chapters(
            &fetch_api_or_fixture(&details_api_url(&key), DETAILS_FIXTURE),
            &key,
        ))
    }
