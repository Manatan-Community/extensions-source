...
const SOURCE: NhatTruyen = NhatTruyen;
const BASE_URL: &str = "https://nhattruyenqq.com";

struct NhatTruyen;

impl MangaSource for NhatTruyen {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        pairs.push(format!(
            "sort={}",
            url::query_escape(filter(filters, "order").unwrap_or("0"))
        ));
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        let target = format!(
            "{BASE_URL}/Comic/Services/ComicService.asmx/ChapterList?slug={}",
...
fn parse_chapters(body: &str, slug: &str) -> Vec<MangaChapter> {
    ...
    response
        ...
        .into_iter()
        .map(|chapter| {
            let key = format!("/truyen-tranh/{slug}/{}", chapter.chapter_slug);
            MangaChapter {
                key: key.clone(),
                title: Some(chapter.chapter_name),
                chapter_number: chapter
                    .chapter_num
                    .or_else(|| chapter_number(&chapter.chapter_slug)),
...
fn chapter_number(slug: &str) -> Option<f32> {
    let number = slug
