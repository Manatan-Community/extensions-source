...
const SOURCE: MeDamTruyen = MeDamTruyen;
const BASE_URL: &str = "https://saytongtai.site";

struct MeDamTruyen;

impl MangaSource for MeDamTruyen {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let genre = request.get("filters").and_then(|f| f.get("genre")).and_then(Value::as_str).unwrap_or_default();
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        let key = manga::request_key(&request, "chapter").unwrap_or_else(|| "/truyen/sample-chap-1".into());
        ...
        if body.contains("post-password-form") || body.contains("post_password") {
            return Ok(vec![manga::text_page("Vui long nhap mat khau chuong nay qua WebView")]);
        }
        let images = extract_image_urls(&body, &chapter_url);
        if images.is_empty() {
            return Ok(vec![manga::text_page("Khong tim thay hinh anh")]);
        }
        Ok(images.into_iter().enumerate().map(|(index, image)| page(index, &image, &chapter_url)).collect())
    }
...
fn parse_popular(body: &str) -> Vec<CatalogItem> {
    body.split("sidebar-comic-block")
        .skip(1)
        .filter_map(|chunk| {
            let href = html::attr_after(chunk, "sidebar-comic-block-link", "href").or_else(|| html::attr_after(chunk, "<a", "href"))?;
            let key = normalize_key(&href);
            let title = html::text_between(chunk, "sidebar-comic-block-title", "</h3>")
                .map(|v| html::strip_tags(&v))
                .filter(|v| !v.is_empty())
                .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Manga".into()));
            Some(catalog_item(key, title, image_attr(chunk).map(|image| absolute_url(&image))))
        })
        .fold(Vec::new(), push_unique)
}

fn parse_listing(body: &str) -> Paged<CatalogItem> {
    let entries = body.split("comic-list-item").skip(1).filter_map(|chunk| {
        let href = html::attr_after(chunk, "comic-block-link", "href").or_else(|| html::attr_after(chunk, "<a", "href"))?;
        let key = normalize_key(&href);
...
fn parse_search_response(body: &str) -> Paged<CatalogItem> {
    ...
    let entries = parsed.data.into_iter().filter_map(|result| {
        let link = result.link?;
        let title = result.title?;
        Some(catalog_item(normalize_key(&link), title, result.img.map(|img| absolute_url(&img.replace("-150x150", "")))))
    }).fold(Vec::new(), push_unique);
...
fn parse_details(body: &str, key: &str) -> CatalogItem {
    CatalogItem {
        ...
        authors: info_value(body, "Tác giả").map(|v| vec![v]).unwrap_or_default(),
        status: parse_status(&html::strip_tags(body)),
        description: html::text_between(body, "hide-long-text", "</div>")
            .map(|v| html::strip_tags(&v).replace("— Xem Thêm —", "").trim().to_string())
            .filter(|v| !v.is_empty()),
        url: Some(absolute_url(key)),
        language: Some("vi".into()),
        content_rating: Some("suggestive".into()),
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    body.split("chapter-item").skip(1).filter_map(|chunk| {
        let href = html::attr_after(chunk, "chapter-link", "href").or_else(|| html::attr_after(chunk, "<a", "href"))?;
        let key = normalize_key(&href);
        ...
        Some(MangaChapter {
            ...
            date_uploaded: html::text_between(chunk, "chapter-meta", "</p>").and_then(|v| parse_chapter_date(&html::strip_tags(&v))),
            url: Some(absolute_url(&key)),
    ...
    if let Some((one, two, three, data_key)) = secrets {
        if let Some(raw) = js_string_after(body, "var htmlContent") {
            ...
            if let Some(decrypted) = decrypt_content(&passphrase, &raw) {
                ...
                let images = images_from_html(&decrypted, base, data_attr.as_deref(), Some((&one, &two, &three)));
...
fn images_from_html(body: &str, base: &str, data_attr: Option<&str>, secrets: Option<(&str, &str, &str)>) -> Vec<String> {
    body.split("<img").skip(1).filter_map(|chunk| {
        ...
        let decoded = match (obfuscated, secrets) {
            (Some(value), Some((one, two, three))) => Some(value.replace(one, ".").replace(two, ":").replace(three, "/")),
            _ => None,
        };
        decoded.or_else(|| image_attr(chunk))
    }).filter(|image| !image.starts_with("data:")).map(|image| url::join_url(base, &image)).fold(Vec::new(), |mut seen, image| {
...
fn decrypt_content(passphrase: &str, encrypted_json_string: &str) -> Option<String> {
    let normalized = encrypted_json_string.replace("\\\\", "\\").replace("\\\"", "\"").replace("\\/", "/");
    ...
    let plaintext = decryptor.decrypt_padded_mut::<Pkcs7>(&mut ciphertext).ok()?;
...
fn pbkdf2_hmac_sha512(password: &[u8], salt: &[u8], iterations: u32, len: usize) -> Vec<u8> {
    ...
    while offset < len {
        ...
        for _ in 1..iterations {
            ...
            for (a, b) in t.iter_mut().zip(&u) { *a ^= *b; }
        }
...
fn js_string_after(body: &str, marker: &str) -> Option<String> {
    ...
    let quote = tail.find(['"', '\'']).map(|idx| tail.as_bytes()[idx] as char)?;
    let after = tail.split_once(quote)?.1;
    Some(after.split(quote).next()?.to_string())
}

fn image_attr(chunk: &str) -> Option<String> {
    html::attr(chunk, "data-src").or_else(|| html::attr(chunk, "src")).or_else(|| html::attr(chunk, "content"))
}
...
fn info_value(body: &str, label: &str) -> Option<String> {
    body.find(label).map(|idx| html::strip_tags(&body[idx..].split("</li>").next().unwrap_or_default()).replace(label, "").trim().to_string()).filter(|v| !v.is_empty())
}

fn parse_status(text: &str) -> ItemStatus {
    let lower = text.to_lowercase();
    if lower.contains("trọn bộ") || lower.contains("hoàn thành") { ItemStatus::Completed }
...
fn parse_chapter_date(text: &str) -> Option<i64> {
    let date = text.split_whitespace().find(|part| part.matches('/').count() == 2)?;
...
fn normalize_key(value: &str) -> String {
    if value.starts_with("http") { value.trim_start_matches(BASE_URL).trim_end_matches('/').to_string() }
...
const SEARCH_FIXTURE: &str = r#"{"success":true,"data":[{"title":"Sample","link":"https://saytongtai.site/truyen/sample","img":"/cover-150x150.jpg"}]}"#;
