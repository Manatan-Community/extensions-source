...
const SOURCE: TruyenHentaiz = TruyenHentaiz;
const BASE_URL: &str = "https://truyenhentaiz.net";

struct TruyenHentaiz;

impl MangaSource for TruyenHentaiz {
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    let entries = body
        ...
        .filter_map(|chunk| {
            ...
            let title = html::text_between(chunk, "card-manga-title", "</")
    ...
    Paged {
        entries,
        has_next_page: vh::has_next(body) || body.contains("data-page"),
    }
}
...
fn parse_details(body: &str, key: &str) -> CatalogItem {
    CatalogItem {
        ...
        title: html::text_between(body, "card-title", "</")
        ...
        tags: body
            ...
            .collect(),
        description: html::text_between(body, "desc", "</p>")
            .map(|v| html::strip_tags(&v))
            .filter(|v| !v.is_empty()),
        status: html::text_between(body, "Status:", "</")
    ...
    body.split("list-group-item")
        ...
        .filter_map(|chunk| {
            ...
            Some(MangaChapter {
                key: key.clone(),
                title: Some(
                    html::text_between(chunk, "fw-bold", "</")
                        .map(|v| html::strip_tags(&v))
                        .or_else(|| vh::title_from(chunk))
...
const LIST_FIXTURE: &str = r#"<section class="container-box-manga"><div class="card card-manga"><a href="/sample"><h2 class="card-manga-title">Sample</h2><img src="/cover.jpg"></a></div></section>"#;
const DETAILS_FIXTURE: &str = r#"<div class="card mb-3"><h3 class="card-title">Sample</h3><img src="/cover.jpg"><p class="desc">Summary</p></div><li class="list-group-item"><a href="/sample/chapter-1"><span class="fw-bold">Chapter 1</span></a></li>"#;
