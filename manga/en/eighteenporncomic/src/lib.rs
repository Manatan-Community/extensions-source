...
const SOURCE: EighteenPornComic = EighteenPornComic;
const BASE_URL: &str = "https://18porncomic.com";

struct EighteenPornComic;

impl MangaSource for EighteenPornComic {
    ...
    let entries = body
        ...
        .filter_map(|chunk| {
            ...
            let title = html::text_between(chunk, "mg_name", "</div>")
                ...
                .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "18 Porn Comic".into()));
            Some(CatalogItem {
                key: key.clone(),
                title,
