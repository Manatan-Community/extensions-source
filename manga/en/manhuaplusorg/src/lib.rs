...
const SOURCE: ManhuaPlusOrg = ManhuaPlusOrg;
const BASE_URL: &str = "https://manhuaplus.org";

struct ManhuaPlusOrg;

impl MangaSource for ManhuaPlusOrg {
...
fn catalog_from_chunk(chunk: &str) -> Option<CatalogItem> {
    ...
    let title = html::text_between(chunk, ".text-center", "</a>")
        ...
        .unwrap_or_else(|| url::slug_from_url(&href).unwrap_or_else(|| "ManhuaPlus".into()));
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, ".a2 header h1", "</h1>")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "ManhuaPlus".into())),
