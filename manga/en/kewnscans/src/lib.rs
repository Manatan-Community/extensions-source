...
const SOURCE: KewnScans = KewnScans;
const BASE_URL: &str = "https://kewnscans.org";

struct KewnScans;

impl MangaSource for KewnScans {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        for genre in selected_genres(request.get("filters")) {
        ...
        Ok(parse_search(
            ...
            query,
            &selected_genres(request.get("filters")),
        ))
    }
...
fn parse_search(body: &str, query: &str, genres: &[String]) -> Paged<CatalogItem> {
    ...
    let entries = body
        ...
        .filter(|chunk| {
            ...
            genres
                .iter()
                .all(|genre| tags.contains(&genre.to_ascii_lowercase()))
        })
        .filter_map(parse_listing_item)
        .fold(Vec::new(), push_unique);
    Paged {
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        description: html::text_between(body, "pre-wrap", "</p>")
            .or_else(|| html::text_between(body, "overflow-hidden", "</p>"))
        ...
        tags: parse_genres(
            body,
            info_value(body, "Type").or_else(|| info_value(body, "Series Type")),
        ),
        url: Some(url::join_url(BASE_URL, &key)),
        language: Some("en".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    body.split("<a")
        .skip(1)
        .filter(|chunk| {
            chunk.contains("href=") && !chunk.contains("Upcoming") && !chunk.contains("Coin")
        })
        .filter(|chunk| {
            chunk.contains("/chapter") || chunk.contains("/read/") || chunk.contains("/series/")
        })
        .filter_map(|chunk| {
    ...
    body.split("<img")
        ...
        .filter_map(|chunk| {
            html::attr(chunk, "uid")
                .and_then(|uid| {
                    cdn.as_ref()
                        .map(|base| format!("{}/{}", base.trim_end_matches('/'), uid))
                })
...
fn info_value(body: &str, label: &str) -> Option<String> {
    body.split("alt=")
        .find(|chunk| {
            chunk.trim_start().starts_with(&format!("\"{label}\""))
                || chunk.trim_start().starts_with(&format!("'{label}'"))
        })
...
fn selected_genres(filters: Option<&Value>) -> Vec<String> {
    match filters.and_then(|filters| filters.get("genre")) {
        Some(Value::Array(values)) => values
            .iter()
            .filter_map(Value::as_str)
            .flat_map(split_values)
            .collect(),
...
const LIST_FIXTURE: &str = r#"<div class="group overflow-hidden grid"><a href="/series/sample" title="Sample Series"><div style="background-image: url('https://cdn.meowing.org/uploads/cover.jpg')"></div></a></div>"#;
const SEARCH_FIXTURE: &str = r#"<div id="searched_series_page"><button title="Sample Series" tags="[&quot;Drama&quot;]"><a href="/series/sample" title="Sample Series"><div style="background-image: url('/cover.jpg')"></div></a></button></div>"#;
const DETAILS_FIXTURE: &str = r#"<div class="grid"><h1>Sample Series</h1><div class="photoURL" style="background-image: url('/cover.jpg')"></div><p class="pre-wrap">Sample description.</p><div alt="Status">Ongoing</div><div alt="Author">Writer</div><div alt="Artist">Artist</div><div alt="Type">Manhwa</div><a href="/series/?genre=drama">Drama</a></div><div id="chapters"><a href="/series/sample/chapter-1"><div class="text-sm">Chapter 1</div><div class="text-xs">Jan 1, 2024</div></a><a href="/series/sample/chapter-2"><img alt="Coin"><div class="text-sm">Chapter 2</div></a></div>"#;
const PAGES_FIXTURE: &str = r#"<script>let realUrl = `https://cdn.meowing.org/uploads`;</script><div id="pages"><img uid="page1.jpg"><img uid="page2.jpg"></div>"#;
