...
const SOURCE: ZinMangaNet = ZinMangaNet;
const CONFIG: MadaraConfig = MadaraConfig {
    base_url: "https://zinmanga.net",
    lang: "en",
    content_rating: "safe",
    manga_path: "manga",
...
impl MangaSource for ZinMangaNet {
...
fn fetch_madara_chapters(key: &str) -> Vec<MangaChapter> {
    ...
    let manga_id = html::attr_after(&body, "manga-chapters-holder", "data-id")
        .or_else(|| html::attr_after(&body, "rating-post-id", "value"));
    let Some(manga_id) = manga_id else {
        return fallback_chapter(key);
    };
        ...
        .filter_map(|chunk| {
            ...
            Some(MangaChapter {
                ...
                date_uploaded: html::text_between(chunk, "chapter-release-date", "</")
                    .map(|value| html::strip_tags(&value))
                    .and_then(|value| parse_slash_date(&value).or_else(|| parse_iso_date(&value))),
...
const DETAILS_FIXTURE: &str = r#"<h1 class="post-title">Sample Manga</h1><div class="summary_image"><img src="/cover.jpg"></div><div class="description-summary">Summary</div><div class="manga-chapters-holder" data-id="123"></div><div class="wp-manga-chapter"><a href="/manga/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">01/01/2024</span></div>"#;
