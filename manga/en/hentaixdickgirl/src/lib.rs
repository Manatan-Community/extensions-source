...
const SOURCE: HentaiXDickgirl = HentaiXDickgirl;

struct HentaiXDickgirl;

impl MangaSource for HentaiXDickgirl {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.starts_with(config.base_url) {
            ...
            return Ok(Paged {
                entries: vec![details_from_body(&body, Some(key), &config)],
                has_next_page: false,
            });
        }
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        ...
        Ok(details_from_body(&body, Some(key), &config))
    }
...
fn details_from_body(body: &str, key: Option<String>, config: &MadaraConfig) -> CatalogItem {
...
fn config() -> MadaraConfig {
    MadaraConfig {
        base_url: "https://hentaixdickgirl.com",
        lang: "en",
...
mod tests {
    ...
    fn parses_madara_source() {
        ...
        assert_eq!(
            SOURCE
                .details(json!({"manga":"/manga/sample"}))
                .unwrap()
                .status,
