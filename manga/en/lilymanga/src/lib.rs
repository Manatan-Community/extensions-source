...
const SOURCE: LilyManga = LilyManga;

struct LilyManga;

impl MangaSource for LilyManga {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let config = config();
        let key = manga::request_key(&request, "manga").unwrap_or_else(|| "/ys/sample".to_string());
        let body =
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        let config = config();
        let key = manga::request_key(&request, "manga").unwrap_or_else(|| "/ys/sample".to_string());
        Ok(fetch_madara_chapters(&config, &key))
    }

    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        let config = config();
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/ys/sample/chapter-1".to_string());
        let body =
...
fn fetch_madara_chapters(config: &MadaraConfig, key: &str) -> Vec<MangaChapter> {
    ...
    let Some(_) = html::attr_after(&body, "manga-chapters-holder", "data-id") else {
        return manga::Madara::parse_chapters(&body, key, config);
    };
...
<div class="page-item-detail"><h3 class="post-title"><a href="/ys/sample/">Sample Manga</a></h3><img src="/cover.jpg"></div>
<div class="nav-previous"></div>
"#;
const DETAILS_FIXTURE: &str = r#"
<h1 class="post-title">Sample Manga</h1><div class="summary_image"><img src="/cover.jpg"></div>
<ul class="main version-chap"><li class="wp-manga-chapter"><a href="/ys/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">01.01.2024</span></li></ul>
"#;
