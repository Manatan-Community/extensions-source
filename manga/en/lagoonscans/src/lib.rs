...
const SOURCE: LagoonScans = LagoonScans;
const BASE_URL: &str = "https://lagoonscans.com";

struct LagoonScans;

impl MangaSource for LagoonScans {
        ...
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                title: html::text_between(chunk, "card-v-title", "</")
                    ...
                    .unwrap_or_else(|| {
                        url::slug_from_url(&key).unwrap_or_else(|| "Lagoon Scans".into())
                    }),
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "manga-title", "</")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Hades Scans".into())),
        cover: html::attr_after(body, "info-poster", "data-src")
            .or_else(|| html::attr_after(body, "info-poster", "src"))
