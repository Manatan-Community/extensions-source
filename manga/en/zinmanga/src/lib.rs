...
const SOURCE: ZinManga = ZinManga;
...
fn fetch_madara_chapters(key: &str) -> Vec<MangaChapter> {
    ...
    let Some(manga_id) = html::attr_after(&body, "rating-post-id", "value") else {
        return fallback_chapter(key);
    };
    let legacy = manga::Madara::browser_client(&CONFIG)
        .post(format!(
        ...
        .send_text()
        .unwrap_or_else(|_| DETAILS_FIXTURE.to_string());
    let chapters = parse_chapter_blocks(&legacy);
    if chapters.is_empty() {
...
const DETAILS_FIXTURE: &str = r#"<h1 class="post-title">Sample Manga</h1><div class="summary_image"><img src="/cover.jpg"></div><div class="description-summary">Summary</div><input class="rating-post-id" value="123"><div class="wp-manga-chapter"><a href="/manga/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">2024-01-01</span></div>"#;
