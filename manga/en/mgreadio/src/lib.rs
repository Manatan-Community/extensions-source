...
const SOURCE: MgreadIo = MgreadIo;
const BASE_URL: &str = "https://mgread.io";

struct MgreadIo;

impl MangaSource for MgreadIo {
...
fn grid_item(chunk: &str) -> Option<CatalogItem> {
    ...
    let title = html::text_between(chunk, "<h2", "</h2>")
        ...
        .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Mgread.io".into()));
    ...
    let entries = rows
        ...
        .filter_map(|row| {
            ...
            let title = json_text(row, "title")
                .map(|value| html::strip_tags(&value))
                .unwrap_or_else(|| "Mgread.io".to_string());
...
const SEARCH_FIXTURE: &str = r#"[{"title":"Sample Manga","url":"https://mgread.io/manga/sample/","thumb":"https://mgread.io/cover.jpg"}]"#;
