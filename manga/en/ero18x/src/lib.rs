...
const SOURCE: Ero18x = Ero18x;
const AJAX_URL: &str = "https://ero18x.com/wp-admin/admin-ajax.php";

struct Ero18x;

impl MangaSource for Ero18x {
...
fn fetch_ajax_chapters(body: &str, manga_key: &str, config: &MadaraConfig) -> Vec<MangaChapter> {
    let Some(post_id) = manga_id(body) else {
        return Vec::new();
    };
    let response = HttpClient::browser()
        ...
        .post(AJAX_URL)
        .xhr()
        .form(&[("action", "manga_get_chapters"), ("manga", &post_id)])
        .send_text()
        .unwrap_or_default();
...
fn manga_id(body: &str) -> Option<String> {
    for marker in [
        "data-post-id=\"",
        "data-manga=\"",
        "manga_id = \"",
        ...
        let value = body.split(marker).nth(1)?.split(['"', '\'']).next()?;
...
<div id="manga-chapters-holder" data-post-id="123"></div>
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(manga_id(DETAILS_FIXTURE).as_deref(), Some("123"));
    }
}
