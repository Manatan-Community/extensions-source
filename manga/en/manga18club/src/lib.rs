...
const SOURCE: Manga18Club = Manga18Club;
const BASE_URL: &str = "https://manga18.club";

struct Manga18Club;

impl MangaSource for Manga18Club {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target =
            if let Some(tag) = filter_value(&request, "tag").filter(|value| !value.is_empty()) {
                let sort = filter_value(&request, "sort").unwrap_or_default();
                list_url(page, &tag, &sort)
            } else {
                format!(
                    "{BASE_URL}/list-manga/{page}?search={}",
    ...
    let entries = body
        ...
        .filter_map(|chunk| {
            ...
            let title = html::text_between(chunk, "mg_name", "</div>")
                ...
                .unwrap_or_else(|| {
                    url::slug_from_url(&key).unwrap_or_else(|| "Manga18.Club".into())
                });
            Some(CatalogItem {
                key: key.clone(),
                title,
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "detail_name", "</h1>")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Manga18.Club".into())),
...
fn details_description(body: &str) -> Option<String> {
    let mut parts = html::text_between(body, "detail_reviewContent", "</div>")
        ...
        .collect::<Vec<_>>();
    if let Some(alt) =
        info_text(body, "Other name").filter(|value| !value.eq_ignore_ascii_case("updating"))
    {
...
"#;
const PAGES_FIXTURE: &str = r#"<script>var slides_p_path = ["L2ltYWdlcy9wYWdlMS5qcGc=","L2ltYWdlcy9wYWdlMi5qcGc=",]</script>"#;
