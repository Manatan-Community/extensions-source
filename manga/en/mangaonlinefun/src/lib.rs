...
const SOURCE: MangaOnlineFun = MangaOnlineFun;
const BASE_URL: &str = "https://mangaonline.fun";
...
const MANGA_SOURCE: &str = "m02";

struct MangaOnlineFun;

impl MangaSource for MangaOnlineFun {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = graphql_or_fixture(&search_query("", "all", order, page), None, SEARCH_FIXTURE);
        ...
        if query.starts_with(BASE_URL) {
            ...
            return Ok(Paged {
                entries: vec![parse_details_response(
                    &graphql_or_fixture(
                        &details_query(slug_from_key(&key)),
                        Some(&absolute_url(&key)),
                        DETAILS_FIXTURE,
                    ),
                    Some(key),
                )],
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        Ok(parse_chapters_response(
            &graphql_or_fixture(
                &chapters_query(slug_from_key(&key)),
                Some(&absolute_url(&key)),
                CHAPTERS_FIXTURE,
            ),
            slug_from_key(&key),
...
fn graphql_or_fixture(query: &str, refresh_url: Option<&str>, fixture: &str) -> String {
    let target = refresh_url.unwrap_or(BASE_URL);
    let _ = client().get(target).browser_document().send_text();
    let access = mhub_access_cookie().unwrap_or_default();
    let body = json!({ "query": query }).to_string();
    let response = client()
        .post(API_URL)
        .header("Accept", "application/json")
        .header("Origin", BASE_URL)
        .header("x-mhub-access", access)
        .json(body)
        .send_text();
    response.unwrap_or_else(|_| fixture.to_string())
}
...
fn mhub_access_cookie() -> Option<String> {
    ...
    response
        .cookies
        .into_iter()
        .find(|cookie| cookie.name == "mhub_access" && !cookie.value.is_empty())
        .map(|cookie| cookie.value)
        .or_else(|| {
            response.header.and_then(|header| {
                header.split(';').find_map(|part| {
                    part.trim()
                        .strip_prefix("mhub_access=")
...
fn search_query(query: &str, genre: &str, order: &str, page: u64) -> String {
    ...
    format!(
        r#"{{ search(x: {MANGA_SOURCE}, q: "{escaped}", genre: "{genre}", mod: {order}, offset: {offset}) {{ rows {{ title author slug image genres latestChapter }} }} }}"#
    )
}

fn details_query(slug: &str) -> String {
    format!(
        r#"{{ manga(x: {MANGA_SOURCE}, slug: "{slug}") {{ title slug status image author artist genres description alternativeTitle }} }}"#
    )
}

fn chapters_query(slug: &str) -> String {
    format!(
        r#"{{ manga(x: {MANGA_SOURCE}, slug: "{slug}") {{ slug chapters {{ number title date }} }} }}"#
    )
}

fn pages_query(slug: &str, number: f32) -> String {
    format!(
        r#"{{ chapter(x: {MANGA_SOURCE}, slug: "{slug}", number: {number}) {{ pages mangaID number manga {{ slug }} }} }}"#
    )
}
    ...
    let entries = rows
        ...
        .filter_map(|row| {
            ...
            let signature = format!(
                "{}{}{}",
                json_text(row, "author").unwrap_or_default(),
                json_number(row, "latestChapter").unwrap_or_default(),
                json_text(row, "genres").unwrap_or_default()
            );
            if seen.iter().any(|value| value == &signature) {
                return None;
            }
            seen.push(signature);
        ...
        .collect::<Vec<_>>();
    Paged {
        has_next_page: rows.len() == 30,
        entries,
    }
}

fn parse_details_response(body: &str, key: Option<String>) -> CatalogItem {
    let manga = serde_json::from_str::<Value>(body)
        .ok()
        .and_then(|root| root.get("data")?.get("manga").cloned())
        .unwrap_or(Value::Null);
    let slug = json_text(&manga, "slug")
        .unwrap_or_else(|| slug_from_key(key.as_deref().unwrap_or("/manga/sample")).to_string());
    let key = key.unwrap_or_else(|| format!("/manga/{slug}"));
    let alt_title = json_text(&manga, "alternativeTitle");
    let mut description = json_text(&manga, "description").unwrap_or_default();
    if let Some(alt_title) = alt_title.filter(|value| !value.is_empty()) {
...
fn parse_chapters_response(body: &str, slug: &str) -> Vec<MangaChapter> {
    let chapters = serde_json::from_str::<Value>(body)
        .ok()
        .and_then(|root| {
            root.get("data")?
                .get("manga")?
    ...
    chapters
        ...
        .map(|chapter| {
            ...
            let display = if title.contains(&number_string) {
                title
            } else if title.trim().is_empty() {
                format!("Chapter {number_string}")
            } else {
                format!("Chapter {number_string} - {}", title.trim())
            };
...
fn parse_pages_response(body: &str) -> Vec<MangaPage> {
    let chapter = serde_json::from_str::<Value>(body)
        .ok()
        .and_then(|root| root.get("data")?.get("chapter").cloned())
        .unwrap_or(Value::Null);
    ...
    pages
        ...
        .map(|(index, image)| MangaPage {
            content: PageContent::Url {
                url: format!("{IMAGE_CDN}/{base}{image}"),
