...
const SOURCE: KissmangaIn = KissmangaIn;

struct KissmangaIn;

impl MangaSource for KissmangaIn {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.starts_with(config.base_url) {
            let key = clean_key(&config.normalize_manga_key(query));
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        ...
        let key = clean_key(
            &manga::request_key(&request, "manga")
                .unwrap_or_else(|| "/kissmanga/sample".to_string()),
        );
        let body =
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        Ok(manga::Madara::parse_chapters(&body, &key, &config)
            .into_iter()
            .filter(|chapter| !chapter.is_locked && !chapter.key.contains("/premium/"))
            .map(|mut chapter| {
                chapter.key = format!("{}?style=list", clean_key(&chapter.key));
                chapter.url = Some(config.absolute_url(&chapter.key));
                chapter
            })
            .collect())
    }
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/kissmanga/sample/chapter-1?style=list".to_string());
        let body =
        ...
        if input.starts_with(config.base_url) {
            return Ok(Some(UrlResolveResult {
                item: Some(manga::Madara::parse_details(
                    ...
                    Some(clean_key(&config.normalize_manga_key(input))),
                    &config,
                )),
...
<div class="page-item-detail"><h3 class="post-title"><a href="/kissmanga/sample/?utm=x">Sample Manga</a></h3><img src="/cover.jpg"></div>
<div class="nav-previous"></div>
"#;
const DETAILS_FIXTURE: &str = r#"
<div class="post-title"><h1>Sample Manga</h1></div><div class="summary_image"><img src="/cover.jpg"></div>
<ul class="main version-chap"><li class="wp-manga-chapter"><a href="/kissmanga/sample/chapter-1/?utm=x">Chapter 1</a><span class="chapter-release-date">2024-01-01</span></li><li class="wp-manga-chapter"><span class="locked-badge"></span><a href="/kissmanga/sample/premium/">Premium</a></li></ul>
"#;
