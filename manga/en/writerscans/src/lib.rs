...
const SOURCE: WriterScans = WriterScans;
const BASE_URL: &str = "https://writerscans.com";

struct WriterScans;

impl MangaSource for WriterScans {
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    let entries = body
        .split("<div")
        .skip(1)
        .filter(|chunk| chunk.contains("group") && chunk.contains("overflow-hidden"))
        .filter(|chunk| chunk.contains("<a") || chunk.contains("href="))
...
fn parse_search(body: &str, query: &str, filters: &Value) -> Paged<CatalogItem> {
    let lower_query = query.to_ascii_lowercase();
    let genres = selected_values(filters.get("genre"));
    let statuses = selected_values(filters.get("status"));
    let types = selected_values(filters.get("type"));
    let entries = body
        .split("<button")
        .skip(1)
        ...
        .filter(|chunk| {
            ...
            let status = html::attr(chunk, "data-status")
                .unwrap_or_default()
                .to_ascii_lowercase();
        ...
        .filter(|chunk| {
            ...
            let entry_type = html::attr(chunk, "data-type")
                .unwrap_or_default()
                .to_ascii_lowercase();
            types.iter().any(|value| value == &entry_type)
        })
        .filter(|chunk| {
            if genres.is_empty() {
                return true;
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "<h1", "</h1>")
            ...
            .filter(|value| !value.is_empty())
            .or_else(|| url::slug_from_url(&key))
            .unwrap_or_else(|| "Writer Scans".to_string()),
        ...
        tags: parse_genres(body, alt_value(body, "Type")),
        url: Some(url::join_url(BASE_URL, &key)),
        language: Some("en".to_string()),
        content_rating: Some("safe".to_string()),
        initialized: true,
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    body.split("<a")
        ...
        .filter(|chunk| chunk.contains("/chapter") || chunk.contains("/read/"))
        .filter_map(|chunk| {
...
<div class="group overflow-hidden"><a href="/series/sample" title="Sample Series"><div style="background-image: url('https://cdn.keyoapp.com/uploads/cover.jpg')"></div></a></div>
"#;
const SEARCH_FIXTURE: &str = r#"
<div id="searched_series_page"><button title="Sample Series" tags="[&quot;Drama&quot;]" data-status="ongoing" data-type="manhwa"><a href="/series/sample" title="Sample Series"><div style="background-image: url('/cover.jpg')"></div></a></button></div>
"#;
const DETAILS_FIXTURE: &str = r#"
<div class="grid"><h1>Sample Series</h1><div class="photoURL" style="background-image: url('/cover.jpg')"></div>
<div class="overflow-hidden"><p>Sample description.</p></div><div alt="Status">Ongoing</div><div alt="Author">Writer</div><div alt="Artist">Artist</div><div alt="Type">Manhwa</div><a href="/series/?genre=drama">Drama</a></div>
...
<script>let realUrl = `https://cdn.keyoapp.com/uploads`;</script>
