...
const SOURCE: HiveScans = HiveScans;
...
impl MangaSource for HiveScans {
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        let show_locked = request
            ...
            .and_then(|prefs| {
                prefs
                    .get("show_locked_chapters")
                    .or_else(|| prefs.get("showLockedChapters"))
            })
...
impl IkenChapter {
    ...
    fn to_chapter(self, parent_slug: Option<&str>) -> MangaChapter {
        let series_slug = parent_slug
            ...
            .unwrap_or("sample");
        let number = chapter_number(&self.number);
        let suffix = self
            .title
            .filter(|value| !value.is_empty())
            .map(|title| format!(" - {title}"))
            .unwrap_or_default();
        let prefix = if self.is_accessible { "" } else { "[Locked] " };
        MangaChapter {
...
const SEARCH_FIXTURE: &str = r#"{
  "posts": [{
    ...
    "isNovel": false,
    "featuredImage": "https://hivetoons.org/cover.jpg",
...
const PAGES_FIXTURE: &str = r#"{
  "chapter": {
    "images": [{ "url": "https://hivetoons.org/page 1.jpg", "order": 1 }],
