...
const BASE_URL: &str = "https://kemono.cr";
const IMG_CDN_URL: &str = "https://img.kemono.cr";
const SOURCE: Kemono = Kemono;

struct Kemono;

impl MangaSource for Kemono {
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let key = manga::request_key(&request, "manga").unwrap_or_else(|| "/fanbox/user/sample".into());
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        let key = manga::request_key(&request, "chapter").unwrap_or_else(|| "/fanbox/user/sample/post/1".into());
...
fn search_creators(page: u64, query: &str, sort: &str) -> Paged<CatalogItem> {
    ...
    creators.retain(|creator| {
        creator.service != "discord"
            && service_allowed(&creator.service)
...
fn creator_key_item(key: &str) -> CatalogItem {
    ...
    let service = parts.first().copied().unwrap_or("fanbox");
...
fn service_allowed(service: &str) -> bool {
    !service.eq_ignore_ascii_case("discord")
}
...
const CREATORS_FIXTURE: &str = r#"[{"id":"sample","name":"Sample Creator","service":"fanbox","updated":"2024-01-01T00:00:00","favorited":10}]"#;
const POSTS_FIXTURE: &str = r#"[{"id":"1","service":"fanbox","user":"sample","title":"Sample Post","added":"2024-01-01T00:00:00","published":null,"edited":null,"file":{"path":"/aa/image.jpg"},"attachments":[]}]"#;
const POST_FIXTURE: &str = r#"{"post":{"id":"1","service":"fanbox","user":"sample","title":"Sample Post","added":"2024-01-01T00:00:00","published":null,"edited":null,"file":{"path":"/aa/image.jpg"},"attachments":[{"path":"/bb/extra.png"}]}}"#;
