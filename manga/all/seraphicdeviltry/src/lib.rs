...
const SOURCE: SeraphicDeviltry = SeraphicDeviltry;

struct SeraphicDeviltry;

impl MangaSource for SeraphicDeviltry {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = config.list_url(page, order);
...
fn config_for(request: &Value) -> MadaraConfig {
    match request
        .get("sourceId")
        .or_else(|| request.get("source_id"))
        .and_then(Value::as_str)
    {
        Some("seraphicdeviltry-es") => MadaraConfig {
            base_url: "https://spanish.seraphic-deviltry.com",
            lang: "es",
        ...
        _ => MadaraConfig {
            base_url: "https://seraphic-deviltry.com",
            lang: "en",
            content_rating: "adult",
            manga_path: "manga",
            popular_url_marker: "post-title",
...
<ul class="main version-chap"><li><a href="/manga/sample/chapter-1/">Chapter 1</a><span class="chapter-release-date">01/01/2024</span></li></ul>
"#;
...
mod tests {
    ...
    fn parses_madara_source() {
        let listing = SOURCE
            .list(json!({"sourceId":"seraphicdeviltry-en"}))
            .unwrap();
        assert_eq!(listing.entries[0].title, "Sample Manga");

        let chapters = SOURCE
            .chapters(json!({"sourceId":"seraphicdeviltry-en","manga":"/manga/sample"}))
            .unwrap();
