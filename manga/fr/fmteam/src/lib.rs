...
const SOURCE: Fmteam = Fmteam;
const BASE_URL: &str = "https://fmteam.fr";
const API_URL: &str = "https://fmteam.fr/api";

struct Fmteam;

impl MangaSource for Fmteam {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if let Some(key) = key_from_input(query) {
            let body = fetch_json(&key, DETAILS_FIXTURE);
            return Ok(Paged {
                entries: parse_detail_item(&body, Some(key)).into_iter().collect(),
                has_next_page: false,
            });
        }
        Ok(parse_comics(
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    let chapters = serde_json::from_str::<PizzaResultDto>(body)
        .ok()
        .and_then(|response| response.comic)
...
const DETAILS_FIXTURE: &str = r#"{"comic":{"title":"Sample","thumbnail":"/cover.jpg","url":"/sample","author":"Auteur","artist":"Artiste","description":"Resume","genres":[{"name":"Action"}],"status":"On going","chapters":[{"chapter":1,"full_title":"Chapitre 1","published_on":"2024-01-01T00:00:00.000000","teams":[{"name":"FMTEAM"}],"url":"/sample/1"}]}}"#;
const PAGES_FIXTURE: &str =
    r#"{"chapter":{"pages":["https://fmteam.fr/page1.jpg","https://fmteam.fr/page2.jpg"]}}"#;
