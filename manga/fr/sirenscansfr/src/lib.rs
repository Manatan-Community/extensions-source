...
const SOURCE: SirenScansFr = SirenScansFr;
const BASE_URL: &str = "https://sirenscans.fr";
const LANG: &str = "fr";
const CONTENT_RATING: &str = "safe";

struct SirenScansFr;

impl MangaSource for SirenScansFr {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let mut entries = parse_cards(&fetch_document(&target, SEARCH_FIXTURE));
        if !query.is_empty() {
...
fn parse_cards(body: &str) -> Vec<CatalogItem> {
    body.split('<')
        .filter(|chunk| {
            (chunk.starts_with("button") || chunk.starts_with("div") || chunk.starts_with("a"))
                && chunk.contains("href")
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    body.split("<a")
        .skip(1)
        .filter(|chunk| chunk.contains("href") && chunk.contains("/chapter"))
        .filter_map(|chunk| {
            let href = html::attr(chunk, "href")?;
...
fn parse_status(input: &str) -> ItemStatus {
    ...
    if lower.contains("completed") || lower.contains("termine") || lower.contains("terminé") {
        ItemStatus::Completed
    } else if lower.contains("dropped") || lower.contains("abandon") {
...
fn cdn_url(body: &str) -> Option<String> {
    let chunk = body.split("realUrl").nth(1)?;
...
const HOME_FIXTURE: &str = r#"<a href="/series/sample" title="Sample Siren" style="background-image:url('https://cdn.keyoapp.com/cover.jpg')"></a>"#;
const SEARCH_FIXTURE: &str = HOME_FIXTURE;
const DETAILS_FIXTURE: &str = r#"<div><h1>Sample Siren</h1></div><div style="background-image:url('https://cdn.keyoapp.com/cover.jpg')"></div><div>Synopsis</div><div>Resume</div><div>Status</div><div>Ongoing</div><a href="?genre=action">Action</a><div id="chapters"><a href="/series/sample/chapter-1"><div class="text-sm">Chapitre 1</div><div class="text-xs">2024-01-01</div></a></div>"#;
const PAGES_FIXTURE: &str =
    r#"<script>realUrl = `https://cdn.keyoapp.com`</script><div><img uid="page1.jpg"></div>"#;
