...
const SOURCE: CovenScan = CovenScan;
const BASE_URL: &str = "https://covendasbruxonas.com";
...
impl MangaSource for CovenScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_covenscan_pages(&body);
        if pages.is_empty() {
...
fn parse_covenscan_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Coven Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Coven Scan</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(parse_covenscan_pages(PAGES_FIXTURE).len(), 2);
    }
}
