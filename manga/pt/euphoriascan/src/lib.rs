...
const SOURCE: EuphoriaScan = EuphoriaScan;
const BASE_URL: &str = "https://euphoriascan.com";
...
impl MangaSource for EuphoriaScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_euphoriascan_pages(&body);
        if pages.is_empty() {
...
fn parse_euphoriascan_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Euphoria Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Euphoria Scan</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        assert_eq!(
            ...
            "Sample Euphoria Scan"
        );
        assert_eq!(parse_euphoriascan_pages(PAGES_FIXTURE).len(), 2);
    }
}
