...
const SOURCE: LeitorDeMangas = LeitorDeMangas;
const BASE_URL: &str = "https://leitordemangas.com";
...
struct LeitorDeMangas;

impl MangaSource for LeitorDeMangas {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_leitordemangas_pages(&body);
        if pages.is_empty() {
...
fn parse_leitordemangas_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Leitor de Mangas</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Leitor de Mangas</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        assert_eq!(
            ...
            "Sample Leitor de Mangas"
        );
        assert_eq!(parse_leitordemangas_pages(PAGES_FIXTURE).len(), 2);
    }
}
