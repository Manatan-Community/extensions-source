...
const SOURCE: Amuy = Amuy;
const BASE_URL: &str = "https://apenasmaisumyaoi.com";
...
impl MangaSource for Amuy {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_amuy_pages(&body);
        if pages.is_empty() {
...
fn parse_amuy_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Amuy</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Amuy</h1>
...
mod tests {
    ...
    #[test]
    fn parses_fixture() {
        assert_eq!(list_from_body(LIST_FIXTURE).entries[0].title, "Sample Amuy");
        assert_eq!(parse_amuy_pages(PAGES_FIXTURE).len(), 2);
    }
}
