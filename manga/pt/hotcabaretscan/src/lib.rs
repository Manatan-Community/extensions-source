...
const SOURCE: HotCabaretScan = HotCabaretScan;
const BASE_URL: &str = "https://hotcabaretscan.com";
...
struct HotCabaretScan;

impl MangaSource for HotCabaretScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_hotcabaretscan_pages(&body);
        if pages.is_empty() {
...
fn parse_hotcabaretscan_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Hot Cabaret Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Hot Cabaret Scan</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        assert_eq!(
            ...
            "Sample Hot Cabaret Scan"
        );
        assert_eq!(parse_hotcabaretscan_pages(PAGES_FIXTURE).len(), 2);
    }
}
