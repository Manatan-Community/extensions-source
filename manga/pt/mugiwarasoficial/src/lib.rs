...
const SOURCE: MugiwarasOficial = MugiwarasOficial;
const BASE_URL: &str = "https://mugiwarasoficial.com";
...
struct MugiwarasOficial;

impl MangaSource for MugiwarasOficial {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_mugiwaras_pages(&body);
        if pages.is_empty() {
...
fn parse_mugiwaras_pages(body: &str) -> Vec<MangaPage> {
        ...
        .and_then(|href| query_param(&CONFIG.absolute_url(&href), "t"))
    {
        ...
        let campaign_body = manga::Madara::browser_client(&CONFIG)
            ...
            .send_text()
            .unwrap_or_default();
        let pages = parse_mugiwaras_image_tags(&campaign_body);
        if !pages.is_empty() {
            return pages;
        }
    }
    parse_mugiwaras_image_tags(body)
}

fn parse_mugiwaras_image_tags(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Mugiwaras</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Mugiwaras</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(parse_mugiwaras_pages(PAGES_FIXTURE).len(), 2);
    }
}
