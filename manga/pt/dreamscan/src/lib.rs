...
const SOURCE: DreamScan = DreamScan;
const BASE_URL: &str = "https://fairydream.com.br";
...
impl MangaSource for DreamScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_dreamscan_pages(&body);
        if pages.is_empty() {
...
fn parse_dreamscan_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Dream Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Dream Scan</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(parse_dreamscan_pages(PAGES_FIXTURE).len(), 2);
    }
}
