...
const SOURCE: CeriseScan = CeriseScan;
const BASE_URL: &str = "https://loverstoon.com";
...
impl MangaSource for CeriseScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_cerisescans_pages(&body);
        if pages.is_empty() {
...
fn parse_cerisescans_pages(body: &str) -> Vec<MangaPage> {
    let script_pages = body
        .split("<script")
        .filter(|chunk| chunk.contains("content"))
        .filter_map(script_content_array)
        .flatten()
        .collect::<Vec<_>>();
    if !script_pages.is_empty() {
        return script_pages
            .into_iter()
            .enumerate()
            .map(|(index, image)| page_from_url(index, &image))
            .collect();
    }
...
fn script_content_array(chunk: &str) -> Option<Vec<String>> {
    let start = chunk.find("content")?;
    let after_content = &chunk[start..];
    let array_start = after_content.find('[')?;
    let after_array = &after_content[array_start..];
    let array_end = after_array.find(']')?;
    serde_json::from_str(&after_array[..=array_end]).ok()
}

fn page_from_url(index: usize, image: &str) -> MangaPage {
    MangaPage {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Cerise Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Cerise Scan</h1>
...
<div class="reading-content">
  <div class="page-break"><script>window.reader = { content: ["/page-1.jpg", "/page-2.jpg"] };</script></div>
</div>
"#;
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(parse_cerisescans_pages(PAGES_FIXTURE).len(), 2);
        assert_eq!(
            script_content_array(r#"content: ["https://cdn.test/page.jpg"]"#)
                .unwrap()
                .len(),
            1
        );
