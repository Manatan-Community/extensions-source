...
const SOURCE: YaoiFanClub = YaoiFanClub;
const BASE_URL: &str = "https://www.yaoifanclub.com";
const NAME: &str = "Yaoi Fan Club";
...
impl MangaSource for YaoiFanClub {
...
fn client() -> http::HttpClient {
    http::HttpClient::browser()
        ...
        .with_referer(format!(
            "https://www.blogger.com/blogin.g?blogspotURL={BASE_URL}/&type=blog&bpli=1"
        ))
...
fn search_labels(request: &Value) -> Vec<String> {
    ...
    labels.extend(selected_value(request, "status"));
    labels.extend(selected_value(request, "type"));
    labels.extend(selected_values(request, "genres"));
    labels
}

fn selected_value(request: &Value, id: &str) -> Option<String> {
    request
        .get("filters")
...
fn chapter_labels(body: &str) -> Vec<String> {
    if let Some(feed) = html::text_between(body, "#latest", "</script>")
        .and_then(|script| quoted_after(&script, "label"))
        .or_else(|| quoted_after(body, "label"))
    {
        return vec![feed];
    }
...
<div id="PopularPosts3"><article><h2 class="post-title"><a href="https://www.yaoifanclub.com/p/sample.html">Sample Yaoi</a></h2><img src="https://blogger.googleusercontent.com/img/s72-c/cover.jpg"></article></div>
"#;
const LIST_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Sample Yaoi"},"category":[{"term":"Series"}],"link":[{"rel":"alternate","href":"https://www.yaoifanclub.com/p/sample.html"}],"media$thumbnail":{"url":"https://blogger.googleusercontent.com/img/s72-c/cover.jpg"}}]}}"#;
const DETAILS_FIXTURE: &str = r#"
<header><img class="thumb" src="https://blogger.googleusercontent.com/img/s72-c/cover.jpg"><h1>Sample Yaoi</h1><span data-status>Ativo</span></header>
<div id="synopsis">Sample description.</div><dt>Genre</dt><dd><a rel="tag">Romance</a></dd><div id="latest"><script>label = 'Sample'</script></div>
"#;
const CHAPTERS_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Capitulo 1"},"category":[{"term":"Chapter"}],"published":{"$t":"2024-01-01T00:00:00.000Z"},"link":[{"rel":"alternate","href":"https://www.yaoifanclub.com/2024/01/sample-chapter-1.html"}]}]}}"#;
