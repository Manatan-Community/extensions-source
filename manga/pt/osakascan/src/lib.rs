...
const SOURCE: OsakaScan = OsakaScan;
const BASE_URL: &str = "https://www.osakascan.com";
const NAME: &str = "Osaka Scan";
...
impl MangaSource for OsakaScan {
...
fn parse_home_listing(body: &str) -> Paged<CatalogItem> {
    let section = body.split("PopularPosts2").nth(1).unwrap_or(body);
    let entries = section
        .split("<article")
        .skip(1)
        .filter(|chunk| chunk.contains("<h3") || chunk.contains("<img") || chunk.contains("href="))
...
fn parse_details(body: &str, key: &str) -> CatalogItem {
    ...
    CatalogItem {
        ...
        tags: body
            ...
            .filter(|value| !value.is_empty())
            .collect(),
        status: if lower.contains("concluído")
            || lower.contains("concluido")
            ...
            || lower.contains("finalizado")
        {
            ...
            ItemStatus::Hiatus
        } else if lower.contains("ativo")
            || lower.contains("andamento")
            || lower.contains("lançamento")
            || lower.contains("lancamento")
        {
            ItemStatus::Ongoing
        } else {
            ItemStatus::Unknown
    ...
    let mut chapters = root
        ...
        .filter(|entry| {
            entry_categories(entry)
                .iter()
                .any(|category| category.eq_ignore_ascii_case("Chapter"))
        })
        .filter_map(|entry| {
            ...
            Some(MangaChapter {
                key: key.clone(),
                chapter_number: title.as_deref().and_then(extract_chapter_number),
                title,
                date_uploaded: entry
                    .get("published")
...
fn entry_categories(entry: &Value) -> Vec<String> {
    entry
        .get("category")
        .and_then(Value::as_array)
        .into_iter()
...
<div id="PopularPosts2"><article><a href="https://www.osakascan.com/p/sample.html"><img src="https://blogger.googleusercontent.com/img/s72-c/cover.jpg"></a><h3>Sample Osaka</h3></article></div>
"#;
const LIST_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Sample Osaka"},"link":[{"rel":"alternate","href":"https://www.osakascan.com/p/sample.html"}],"media$thumbnail":{"url":"https://blogger.googleusercontent.com/img/s72-c/cover.jpg"}}]}}"#;
const DETAILS_FIXTURE: &str = r#"
<h1>Sample Osaka</h1><meta property="og:image" content="https://blogger.googleusercontent.com/img/s72-c/cover.jpg">
<div id="synopsis">Sample description.</div><span data-status="Lançamento"></span><div class="grid gta-series"><dt>Gênero</dt><dd><a rel="tag">Acao</a></dd></div><div class="chapter_get" data-labelchapter="Sample"></div>
"#;
const CHAPTERS_FIXTURE: &str = r#"{"feed":{"entry":[{"title":{"$t":"Capitulo 1"},"category":[{"term":"Chapter"}],"published":{"$t":"2024-01-01T00:00:00.000Z"},"link":[{"rel":"alternate","href":"https://www.osakascan.com/2024/01/sample-chapter-1.html"}]}]}}"#;
