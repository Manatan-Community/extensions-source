...
const SOURCE: MangasBrasuka = MangasBrasuka;
const BASE_URL: &str = "https://mangasbrasuka.com.br";
...
struct MangasBrasuka;

impl MangaSource for MangasBrasuka {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_mangasbrasuka_pages(&body);
        if pages.is_empty() {
...
fn parse_mangasbrasuka_pages(body: &str) -> Vec<MangaPage> {
    if let Some(redirect) = body
        .split("page-break")
        .nth(1)
        .and_then(|chunk| html::attr_after(chunk, "<a", "href"))
        .and_then(|href| query_param(&CONFIG.absolute_url(&href), "t"))
    {
        let campaign = format!("{BASE_URL}/campanha.php?auth={}", url::query_escape(&redirect));
        let campaign_body = manga::Madara::browser_client(&CONFIG)
            .get(campaign)
            .browser_document()
            .send_text()
            .unwrap_or_default();
        let pages = parse_mangasbrasuka_image_tags(&campaign_body);
        if !pages.is_empty() {
            return pages;
        }
    }
    parse_mangasbrasuka_image_tags(body)
}

fn parse_mangasbrasuka_image_tags(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Mangas Brasuka</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Mangas Brasuka</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        assert_eq!(
            ...
            "Sample Mangas Brasuka"
        );
        assert_eq!(parse_mangasbrasuka_pages(PAGES_FIXTURE).len(), 2);
    }
}
