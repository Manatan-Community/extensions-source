...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        description: html::text_between(body, "entry-content", "</div>")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty()),
        authors: info_values(body, &["Author", "Pengarang", "Mangaka", "ผู้เขียน"]),
        artists: info_values(body, &["artist", "Artist", "seniman", "ศิลปิน"]),
...
fn script_images(body: &str) -> Vec<String> {
    if let Some(encoded) = data_script_base64(body) {
        if let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(encoded) {
            if let Ok(decoded) = String::from_utf8(bytes) {
                let images = script_images_from_plain(&decoded);
                if !images.is_empty() {
                    return images;
                }
            }
    ...
    script_images_from_plain(body)
}

fn script_images_from_plain(body: &str) -> Vec<String> {
...
fn data_script_base64(body: &str) -> Option<&str> {
...
<div id="readerarea"><img src="https://example.invalid/page-1.jpg" alt="Page 1"></div>
"#;
