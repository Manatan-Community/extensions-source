...
const SOURCE: GhostScan = GhostScan;
const BASE_URL: &str = "https://ghostscan.xyz";
...
impl MangaSource for GhostScan {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        let pages = parse_ghostscan_pages(&body);
        if pages.is_empty() {
...
fn parse_ghostscan_pages(body: &str) -> Vec<MangaPage> {
    body.split("<img")
        .skip(1)
        .filter(|chunk| {
...
<div class="page-item-detail">
  <h3 class="post-title"><a href="/manga/sample/">Sample Ghost Scan</a></h3>
  <img src="/cover.jpg">
</div>
...
<h1 class="post-title">Sample Ghost Scan</h1>
...
mod tests {
    ...
    fn parses_fixture() {
        ...
        assert_eq!(parse_ghostscan_pages(PAGES_FIXTURE).len(), 2);
    }
}
