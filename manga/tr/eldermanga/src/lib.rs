...
const SOURCE: ElderManga = ElderManga;
const BASE_URL: &str = "https://eldermanga.com";
const CDN_URL: &str = "https://eldermangacdn2.efsaneler.can.re";

struct ElderManga;

impl MangaSource for ElderManga {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = client()
            .get(format!(
                "{BASE_URL}/api/series/search/navbar?search={}",
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let key =
            manga::request_key(&request, "manga").unwrap_or_else(|| "/manga/1/sample".to_string());
        Ok(parse_details(
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        let key = manga::request_key(&request, "chapter")
            .unwrap_or_else(|| "/manga/1/sample/chapter-1".to_string());
        Ok(parse_pages(
...
fn parse_listing(body: &str, popular: bool) -> Paged<CatalogItem> {
    let chunks: Vec<&str> = if popular {
        ...
        body.split("<div")
            .skip(1)
            .filter(|chunk| {
                chunk.contains("grid") || chunk.contains("card-image") || chunk.contains("/manga/")
            })
            .collect()
    };
    let entries = chunks
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        cover: html::attr_after(body, "#content", "src")
            .or_else(|| image(body))
            .map(|value| absolute_url(&value)),
        description: html::text_between(body, "div.grid h2 + p", "</p>")
        ...
        tags: body
            .split("<a")
            .skip(1)
            .filter(|chunk| chunk.contains("search?categories"))
            .filter_map(|chunk| {
                html::text_between(chunk, ">", "</a>").map(|value| html::strip_tags(&value))
            })
            .collect(),
        status: parse_status(
            &html::text_between(body, "Durum", "</span>")
                .map(|value| html::strip_tags(&value))
                .unwrap_or_default(),
        ),
        url: Some(absolute_url(&key)),
    ...
    body.split("<a")
        ...
        .filter_map(|chunk| {
            ...
            Some(MangaChapter {
                ...
                date_uploaded: html::text_between(chunk, "<span", "</span>").and_then(|value| {
                    manatan_shared::dates::parse_fixture_date(&html::strip_tags(&value))
                }),
...
impl SearchDto {
    fn into_item(self) -> CatalogItem {
        ...
        CatalogItem {
            key: format!("/manga/{}/{slug}", self.id),
            title: self.name,
            cover: Some(if self.image.starts_with("http") {
                self.image
            } else {
                format!(
                    "{}/{}",
            ...
            url: Some(format!("{BASE_URL}/manga/{}/{slug}", self.id)),
            language: Some("tr".into()),
            content_rating: Some("safe".into()),
            initialized: false,
            ..CatalogItem::default()
...
fn parse_status(value: &str) -> ItemStatus {
    ...
    if value.contains("tamamlandi") || value.contains("tamamland\u{131}") {
        ItemStatus::Completed
    } else if value.contains("ara ver") {
        ItemStatus::Hiatus
    } else if value.contains("birakildi") || value.contains("b\u{131}rak\u{131}ld\u{131}") {
        ItemStatus::Cancelled
    } else if value.contains("devam") {
        ItemStatus::Ongoing
    } else {
        ItemStatus::Unknown
    }
...
const PAGES_FIXTURE: &str =
    r#"<script>{\"path\":\"series/sample/001.jpg\"},{\"path\":\"series/sample/002.jpg\"}</script>"#;
