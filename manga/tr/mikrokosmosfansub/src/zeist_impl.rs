...
impl MangaSource for Source {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let page = request.get("page").and_then(Value::as_u64).unwrap_or(1);
        let genre = selected_genre(&request);
        let label = genre.as_deref().unwrap_or(MANGA_CATEGORY);
        let body = fetch_text_or_fixture(&feed_url(label, page, query, Some(label)), FEED_FIXTURE);
        Ok(parse_feed(&body))
    }
...
fn selected_genre(request: &Value) -> Option<String> {
    request
        .get("filters")
...
fn parse_popular(body: &str) -> Paged<CatalogItem> {
    let mut entries = body
        ...
        .collect::<Vec<_>>();
    entries.extend(body.split("<li").skip(1).filter_map(|chunk| {
        if !(chunk.contains("gallery") || chunk.contains("bg")) {
            return None;
        }
        let href = html::attr_after(chunk, "<a", "href")?;
        let key = normalize_key(&href);
        Some(CatalogItem {
            ...
            title: html::text_between(chunk, "<h3", "</h3>")
                ...
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| NAME.into())),
            cover: html::attr_after(chunk, "style", "background-image")
                .or_else(|| html::attr_after(chunk, "style", "background"))
                .map(|value| {
                    value
                        .trim()
...
const FEED_FIXTURE: &str = r#"{
  "feed": {"entry": [{"title":{"$t":"Sample Manga"},"category":[{"term":"Series"}],"link":[{"rel":"alternate","href":"https://example.invalid/2024/01/sample.html"}],"media$thumbnail":{"url":"https://img/cover.jpg"}}]}
}"#;

const CHAPTER_FEED_FIXTURE: &str = r#"{
  "feed": {"entry": [{"title":{"$t":"Chapter 1"},"published":{"$t":"2024-01-01T00:00:00.000Z"},"category":[{"term":"Chapter"}],"link":[{"rel":"alternate","href":"https://example.invalid/2024/01/chapter-1.html"}]}]}
}"#;
