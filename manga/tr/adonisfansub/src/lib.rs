use manatan_extension::{CatalogItem, MangaChapter, MangaPage, Paged, UrlResolveResult, abi::ExtensionResult, export_manga_source, source::MangaSource};
use manatan_shared::{manga, manga::MadaraConfig, sdk::SearchRequest, url};
use serde_json::Value;

const SOURCE: AdonisFansub = AdonisFansub;

struct AdonisFansub;

impl MangaSource for AdonisFansub {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let config = config();
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(Paged { entries: manga::Madara::parse_listing(LIST_FIXTURE, &config), has_next_page: manga::Madara::has_next_page(LIST_FIXTURE, &config) });
        }
        let page = request.get("page").and_then(Value::as_u64).unwrap_or(1);
        let order = if request.get("listingId").and_then(Value::as_str) == Some("latest") { "latest" } else { "views" };
        let body = manga::Madara::fetch_document_or_fixture(&config, &format!("{}/manga/page/{page}/?m_orderby={order}", config.base_url), LIST_FIXTURE);
        Ok(Paged { entries: manga::Madara::parse_listing(&body, &config), has_next_page: manga::Madara::has_next_page(&body, &config) })
    }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.starts_with(config.base_url) {
            ...
            return Ok(Paged { entries: vec![manga::Madara::parse_details(&body, Some(key), &config)], has_next_page: false });
        }
    ...
    fn handle_url(&self, request: Value) -> ExtensionResult<Option<UrlResolveResult>> {
        ...
        if input.starts_with(config.base_url) {
            return Ok(Some(UrlResolveResult { item: Some(manga::Madara::parse_details(&manga::Madara::fetch_document_or_fixture(&config, input, DETAILS_FIXTURE), Some(config.normalize_manga_key(input)), &config)), url: Some(input.to_string()), ..UrlResolveResult::default() }));
        }
...
fn config() -> MadaraConfig {
    MadaraConfig { base_url: "https://manga.adonisfansub.com", lang: "tr", content_rating: "safe", manga_path: "manga", popular_url_marker: "post-title", use_load_more: false, latest_enabled: true }
}
