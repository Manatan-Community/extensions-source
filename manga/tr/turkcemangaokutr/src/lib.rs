...
const SOURCE: TurkceMangaOkuTr = TurkceMangaOkuTr;
struct TurkceMangaOkuTr;

impl manga::MadaraSource for TurkceMangaOkuTr {
    fn madara_config(&self, _request: &serde_json::Value) -> MadaraConfig {
        MadaraConfig { base_url: "https://turkcemangaoku.com.tr", lang: "tr", content_rating: "adult", manga_path: "manga", popular_url_marker: "post-title", use_load_more: false, latest_enabled: true }
    }
    fn madara_list_fixture(&self) -> &'static str { LIST_FIXTURE }
    fn madara_details_fixture(&self) -> &'static str { DETAILS_FIXTURE }
    fn madara_pages_fixture(&self) -> &'static str { PAGES_FIXTURE }
}

impl_madara_source!(TurkceMangaOkuTr);
