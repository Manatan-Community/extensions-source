...
fn parse_pages(body: &str, chapter_url: &str) -> Vec<MangaPage> {
    if body.contains("readerarea")
        && (body.contains("<form") || body.contains("value=\"Doğrula\""))
    {
        return vec![MangaPage {
            content: PageContent::Text {
                text: "Open WebView and solve the captcha to view this chapter".to_string(),
            },
            description: Some("Captcha required".to_string()),
            ..MangaPage::default()
        }];
    }
    let pages = body
