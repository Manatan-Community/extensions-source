...
const SOURCE: SundayWebEvery = SundayWebEvery;
const BASE_URL: &str = "https://www.sunday-webry.com";
const SOURCE_NAME: &str = "Sunday Web Every";

struct SundayWebEvery;

impl MangaSource for SundayWebEvery {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_series_list(LIST_FIXTURE));
        }
        parse_series_list_result(&fetch_document(&format!("{BASE_URL}/series"))?)
    }
...
<li class="series-list"><a href="/series/sample"><img src="/cover.jpg" alt="Sample Sunday Web Every"><h4 class="series-title">Sample Sunday Web Every</h4></a></li>
"#;
