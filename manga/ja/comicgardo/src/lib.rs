...
const SOURCE: ComicGardo = ComicGardo;
const BASE_URL: &str = "https://comic-gardo.com";
const SOURCE_NAME: &str = "Comic Gardo";
const DEFAULT_COLLECTION: &str = "";

struct ComicGardo;

impl MangaSource for ComicGardo {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_series_list(LIST_FIXTURE));
        }
        let target = if DEFAULT_COLLECTION.is_empty() {
            format!("{BASE_URL}/series")
        } else {
            format!("{BASE_URL}/series/{DEFAULT_COLLECTION}")
        };
...
<li class="series-list"><a href="/series/sample"><img src="/cover.jpg" alt="Sample ComicGardo"><h4 class="series-title">Sample ComicGardo</h4></a></li>
"#;
