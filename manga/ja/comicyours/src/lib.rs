...
const SOURCE: ComicYours = ComicYours;
const BASE_URL: &str = "https://comic-y-ours.com";
const SOURCE_NAME: &str = "Comic Y-OURs";
const DEFAULT_COLLECTION: &str = "";

struct ComicYours;

impl MangaSource for ComicYours {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_series_list(LIST_FIXTURE));
        }
        parse_series_list_result(&fetch_document(BASE_URL)?)
    }
...
<a class="MainVisual_imageLink_hash" href="/series/sample" data-series-name="Sample Comic Y-OURs"><img src="/cover.jpg" alt="Sample Comic Y-OURs"></a>
"#;
