...
const SOURCE: ComicBorder = ComicBorder;
const BASE_URL: &str = "https://comicborder.com";

struct ComicBorder;

impl MangaSource for ComicBorder {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_top_series(LIST_FIXTURE));
        }
        Ok(parse_top_series(&fetch_document(BASE_URL, LIST_FIXTURE)))
    }
...
fn parse_top_series(body: &str) -> Paged<CatalogItem> {
    let entries = body
        .split("<section")
        .skip(1)
        .filter(|chunk| chunk.contains("top-series"))
        .filter_map(|chunk| {
            let href = html::attr_after(chunk, "top-series-nav", "href")
                .or_else(|| html::attr_after(chunk, "<a", "href"))?;
            let title = html::text_between(chunk, "<h3", "</h3>")
                .map(|value| html::strip_tags(&value))
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| {
                    url::slug_from_url(&href).unwrap_or_else(|| "Comic Border".into())
                });
            Some(CatalogItem {
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(&info, "series-header-title", "</")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Comic Border".into())),
...
<section class="top-series"><div class="top-series-nav"><a href="/series/sample">Read</a></div><h3>Sample Comic Border</h3><img class="top-key-image" data-src="/cover.jpg"></section>
"#;
const SEARCH_FIXTURE: &str = r#"
<ul class="search-series-list"><li><div class="thmb-container"><a href="/series/sample"><img src="/cover.jpg"></a></div><div class="title-box"><p class="series-title">Sample Comic Border</p></div></li></ul>
"#;
const DETAILS_FIXTURE: &str = r#"
<section class="series-information"><div class="series-header"><h1 class="series-header-title">Sample Comic Border</h1><h2 class="series-header-author">Sample Author</h2><p class="series-header-description">Sample description.</p><div class="series-header-image-wrapper"><img data-src="/cover.jpg"></div><div class="js-readable-products-pagination" data-aggregate-id="sample"></div></div></section>
"#;
