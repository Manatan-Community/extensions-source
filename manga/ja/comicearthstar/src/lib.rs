...
const SOURCE: ComicEarthStar = ComicEarthStar;
const BASE_URL: &str = "https://comic-earthstar.com";
const GRAPHQL_URL: &str = "https://comic-earthstar.com/graphql";

struct ComicEarthStar;

impl MangaSource for ComicEarthStar {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_earthstar_entries(LIST_GRAPHQL_FIXTURE));
        }
        Ok(parse_earthstar_entries(&post_graphql(
            "Earthstar_LatestUpdates",
            LATEST_QUERY,
            json!({
                "latestUpdatedSince": "2000-01-01T00:00:00Z",
                "latestUpdatedUntil": "2999-12-31T23:59:59Z"
            }),
            LIST_GRAPHQL_FIXTURE,
        )))
    }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if !query.is_empty() {
            return Ok(parse_earthstar_entries(&post_graphql(
                "Common_Search",
                SEARCH_QUERY,
                json!({"keyword": query}),
                SEARCH_GRAPHQL_FIXTURE,
            )));
        }
        let (operation, graphql) = match filter_string(&request, "category").unwrap_or("ongoing") {
            "finished" => ("Earthstar_SeriesFinished", FINISHED_QUERY),
            "oneshot" => ("Earthstar_Oneshot", ONESHOT_QUERY),
            _ => ("Earthstar_SeriesOngoing", ONGOING_QUERY),
        };
        Ok(parse_earthstar_entries(&post_graphql(
            operation,
            graphql,
...
fn post_graphql(operation: &str, query: &str, variables: Value, fixture: &str) -> String {
    client()
        .post(GRAPHQL_URL)
        .json(
...
fn parse_earthstar_entries(body: &str) -> Paged<CatalogItem> {
    let root = serde_json::from_str::<Value>(body)
        .unwrap_or_else(|_| serde_json::from_str(LIST_GRAPHQL_FIXTURE).unwrap_or(Value::Null));
    let mut entries = Vec::new();
    for pointer in [
        "/data/serialGroup/latestUpdatedSeriesEpisodes",
        "/data/searchSeries/edges",
        "/data/serialGroup/seriesSlice/seriesList",
    ] {
        for value in root
            .pointer(pointer)
            ...
            .flatten()
        {
            ...
            let series = node.get("series").unwrap_or(node);
            let permalink = node
                .get("permalink")
                .or_else(|| series.pointer("/firstEpisode/permalink"))
                .and_then(Value::as_str)
...
fn parse_daily_series(body: &str, latest: bool) -> Paged<CatalogItem> {
    let entries = body
        .split("<li")
        .skip(1)
        .filter(|chunk| chunk.contains("daily-series-item") && chunk.contains("a"))
        .filter(|chunk| !latest || chunk.contains("daily"))
        .filter_map(|chunk| {
            let href = html::attr_after(chunk, "<a", "href")?;
            let title = html::text_between(chunk, "daily-series-title", "</")
                .or_else(|| html::text_between(chunk, "<h4", "</h4>"))
                .map(|value| html::strip_tags(&value))
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| {
                    url::slug_from_url(&href).unwrap_or_else(|| "Comic Days".into())
                });
            Some(CatalogItem {
...
fn parse_search(body: &str) -> Paged<CatalogItem> {
    let entries = body
        .split("<li")
        .skip(1)
        .filter(|chunk| chunk.contains("search-series-list") || chunk.contains("series-list"))
        .filter_map(|chunk| {
            let href = html::attr_after(chunk, "thmb-container", "href")
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(&info, "series-header-title", "</")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Comic Days".into())),
...
const LATEST_QUERY: &str = r#"query Earthstar_LatestUpdates($latestUpdatedSince: DateTime!, $latestUpdatedUntil: DateTime!) { serialGroup(groupName: "トップ：更新作品") { latestUpdatedSeriesEpisodes: updatedFreeEpisodes(since: $latestUpdatedSince until: $latestUpdatedUntil) { permalink series { title thumbnailUri } } } }"#;
const SEARCH_QUERY: &str = r#"query Common_Search($keyword: String!) { searchSeries(keyword: $keyword) { edges { node { title thumbnailUri firstEpisode { permalink } } } } }"#;
const ONESHOT_QUERY: &str = r#"query Earthstar_Oneshot { serialGroup(groupName: "連載・読切：読切作品") { seriesSlice { seriesList { thumbnailUri title firstEpisode { permalink } } } } }"#;
const ONGOING_QUERY: &str = r#"query Earthstar_SeriesOngoing { serialGroup(groupName: "連載・読切：連載作品：連載中") { seriesSlice { seriesList { thumbnailUri title firstEpisode { permalink } } } } }"#;
const FINISHED_QUERY: &str = r#"query Earthstar_SeriesFinished { serialGroup(groupName: "連載・読切：連載作品：連載終了") { seriesSlice { seriesList { thumbnailUri title firstEpisode { permalink } } } } }"#;
const LIST_GRAPHQL_FIXTURE: &str = r#"{"data":{"serialGroup":{"latestUpdatedSeriesEpisodes":[{"permalink":"/episode/sample","series":{"title":"Sample Comic Earth Star","thumbnailUri":"/cover.jpg"}}]}}}"#;
const SEARCH_GRAPHQL_FIXTURE: &str = r#"{"data":{"searchSeries":{"edges":[{"node":{"title":"Sample Comic Earth Star","thumbnailUri":"/cover.jpg","firstEpisode":{"permalink":"/episode/sample"}}}]}}}"#;
const LIST_FIXTURE: &str = r#"
<ul class="daily-series"><li class="daily-series-item"><a class="link" href="/series/sample"><div class="daily-series-thumb"><img data-src="/cover.jpg"></div><h4 class="daily-series-title">Sample Comic Earth Star</h4></a></li></ul>
"#;
const SEARCH_FIXTURE: &str = r#"
<ul class="search-series-list"><li><div class="thmb-container"><a href="/series/sample"><img src="/cover.jpg"></a></div><div class="title-box"><p class="series-title">Sample Comic Earth Star</p></div></li></ul>
"#;
const DETAILS_FIXTURE: &str = r#"
<section class="series-information"><div class="series-header"><h1 class="series-header-title">Sample Comic Earth Star</h1><h2 class="series-header-author">Sample Author</h2><p class="series-header-description">Sample description.</p><div class="series-header-image-wrapper"><img data-src="/cover.jpg"></div><div class="js-readable-products-pagination" data-aggregate-id="sample"></div></div></section>
"#;
