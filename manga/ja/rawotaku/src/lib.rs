...
const SOURCE: RawOtaku = RawOtaku;
const BASE_URL: &str = "https://rawotaku.com";

struct RawOtaku;

impl MangaSource for RawOtaku {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        for id in ["type", "status", "language"] {
            if let Some(value) =
                filter_string(&request, id).filter(|value| !value.is_empty() && *value != "all")
            {
        ...
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                title: image_attr(chunk, "alt")
                    .or_else(|| {
                        html::text_between(chunk, "post-title", "</")
                            .map(|value| html::strip_tags(&value))
                    })
                    .or_else(|| {
                        html::text_between(chunk, "<h", "</h").map(|value| html::strip_tags(&value))
                    })
                    .filter(|value| !value.is_empty())
                    .unwrap_or_else(|| {
                        url::slug_from_url(&key).unwrap_or_else(|| "Raw Otaku".into())
                    }),
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "<h1", "</h1>")
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Raw Otaku".into())),
        cover: html::attr_after(body, "summary_image", "src")
            .or_else(|| image_attr(body, "src"))
...
fn parse_chapters(body: &str) -> Vec<MangaChapter> {
    ...
    area.split("<a")
        ...
        .filter_map(|chunk| {
            ...
            if let Some(id) =
                html::attr(chunk, "data-id").or_else(|| html::attr(chunk, "data-chapter"))
            {
...
fn parse_pages(body: &str, chapter_key: &str) -> Vec<MangaPage> {
    ...
    if let Some(id) = chapter_id {
        let ajax = fetch_json(
            &format!("{BASE_URL}/json/chapter?mode=vertical&id={id}"),
            AJAX_FIXTURE,
        );
...
fn parse_ajax_pages(body: &str) -> Vec<MangaPage> {
    let value = serde_json::from_str::<Value>(body)
        .unwrap_or_else(|_| serde_json::from_str(AJAX_FIXTURE).unwrap());
...
const LIST_FIXTURE: &str = r#"<div class="page-item-detail"><a href="/manga/sample"><img src="/cover.jpg" alt="Sample Raw Otaku"><h3 class="post-title">Sample Raw Otaku</h3></a></div>"#;
const SEARCH_FIXTURE: &str = LIST_FIXTURE;
const DETAILS_FIXTURE: &str = r#"
<h1>Sample Raw Otaku</h1><div class="summary_image"><img src="/cover.jpg"></div><div class="summary__content">Sample description.</div>
...
"#;
const AJAX_FIXTURE: &str =
    r#"{"images":["https://rawotaku.com/page1.jpg","https://rawotaku.com/page2.jpg"]}"#;
