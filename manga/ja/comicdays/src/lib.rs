...
const SOURCE: ComicDays = ComicDays;
const BASE_URL: &str = "https://comic-days.com";

struct ComicDays;

impl MangaSource for ComicDays {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_daily_series(LIST_FIXTURE, false));
        }
        let latest = request.get("listingId").and_then(Value::as_str) == Some("latest");
        let body = fetch_document(&format!("{BASE_URL}/series"), LIST_FIXTURE);
        Ok(parse_daily_series(&body, latest))
    }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.is_empty() {
            let collection = filter_string(&request, "collection").unwrap_or_default();
            let body = fetch_document(&format!("{BASE_URL}/series/{collection}"), LIST_FIXTURE);
            return Ok(parse_daily_series(&body, false));
        }
...
<ul class="daily-series"><li class="daily-series-item"><a class="link" href="/series/sample"><div class="daily-series-thumb"><img data-src="/cover.jpg"></div><h4 class="daily-series-title">Sample Comic Days</h4></a></li></ul>
"#;
const SEARCH_FIXTURE: &str = r#"
<ul class="search-series-list"><li><div class="thmb-container"><a href="/series/sample"><img src="/cover.jpg"></a></div><div class="title-box"><p class="series-title">Sample Comic Days</p></div></li></ul>
"#;
const DETAILS_FIXTURE: &str = r#"
<section class="series-information"><div class="series-header"><h1 class="series-header-title">Sample Comic Days</h1><h2 class="series-header-author">Sample Author</h2><p class="series-header-description">Sample description.</p><div class="series-header-image-wrapper"><img data-src="/cover.jpg"></div><div class="js-readable-products-pagination" data-aggregate-id="sample"></div></div></section>
"#;
