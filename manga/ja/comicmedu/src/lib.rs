...
const SOURCE: ComicMeDu = ComicMeDu;
const BASE_URL: &str = "https://comic-medu.com";

struct ComicMeDu;

impl MangaSource for ComicMeDu {
        ...
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                title: html::text_between(chunk, "title-text", "</")
                    ...
                    .unwrap_or_else(|| {
                        url::slug_from_url(&key).unwrap_or_else(|| "Comic MeDu".into())
                    }),
...
fn details_by_key(key: &str) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(&body, "series-h-title", "</h1>")
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Comic MeDu".into())),
...
const LIST_FIXTURE: &str = r#"<div class="ranking-box-vertical"><a href="/series/sample"><picture><source data-srcset="//cdn-public.comici.jp/cover.jpg 1x"></picture><div class="title-text">Sample Comic MeDu</div></a></div>"#;
const DETAILS_FIXTURE: &str = r#"<h1 class="series-h-title"><span>Sample Comic MeDu</span></h1><div class="series-h-img"><source data-srcset="//cdn-public.comici.jp/cover.jpg 1x"></div><div class="series-h-credit-user">Sample Author</div><div class="series-h-credit-info-text-text">Sample description.</div><a class="series-h-tag-link">#Action</a>"#;
