...
const SOURCE: ComicRide = ComicRide;
const BASE_URL: &str = "https://comicride.jp";
const API_URL: &str = "https://comicride.jp/api";
const SEARCH_PAGE_SIZE: u64 = 24;

struct ComicRide;

impl MangaSource for ComicRide {
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        Ok(fetch_viewer_pages(&viewer_id, API_URL, user_id.as_deref()))
    }
        ...
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                title: html::text_between(chunk, "series-list-item-h", "</")
                    ...
                    .unwrap_or_else(|| {
                        url::slug_from_url(&key).unwrap_or_else(|| "Comic Ride".into())
                    }),
...
fn fetch_user_id() -> Option<String> {
    let body = fetch_json(&format!("{API_URL}/user/info"), USER_FIXTURE);
...
fn fetch_viewer_pages(viewer_id: &str, api_url: &str, user_id: Option<&str>) -> Vec<MangaPage> {
    let user_id = user_id.map(url::query_escape).unwrap_or_default();
    let first = fetch_json(
        &format!(
            "{api_url}/book/contentsInfo?comici-viewer-id={viewer_id}&user-id={user_id}&page-from=0&page-to=1"
        ),
        VIEWER_FIXTURE,
    );
    ...
    let body = fetch_json(
        &format!(
            "{api_url}/book/contentsInfo?comici-viewer-id={viewer_id}&user-id={user_id}&page-from=0&page-to={total}"
        ),
        VIEWER_FIXTURE,
    );
...
const LIST_FIXTURE: &str = r#"<div class="series-list-item"><a class="series-list-item-link" href="/series/sample"><img class="series-list-item-img" src="/cover.jpg"><div class="series-list-item-h"><span>Sample Comic Ride</span></div></a></div>"#;
const SEARCH_FIXTURE: &str = r#"{"searchResult":{"series":{"total":1,"series":[{"id":"sample","name":"Sample Comic Ride","images":[{"url":"https://img.example.test/cover.jpg"}]}]}}}"#;
const DETAILS_FIXTURE: &str = r#"{"series":{"summary":{"name":"Sample Comic Ride","description":"Sample description.","author":[{"name":"Sample Author"}],"images":[{"url":"https://img.example.test/cover.jpg"}],"tag":[{"name":"Action"}],"isCompleted":false},"episodes":[{"id":"sample","title":"Episode 1","datePublished":1704067200}]}}"#;
