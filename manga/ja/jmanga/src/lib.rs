...
const SOURCE: Jmanga = Jmanga;
const BASE_URL: &str = "https://jmanga.codes";

struct Jmanga;

impl MangaSource for Jmanga {
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = fetch_document(
            &format!("{BASE_URL}/search/story/{}/{page}", url::query_escape(query)),
            LIST_FIXTURE,
        );
        Ok(Paged {
            entries: parse_cards(&body),
    ...
    fn chapters(&self, request: Value) -> ExtensionResult<Vec<MangaChapter>> {
        ...
        let manga_id = body
            ...
            .unwrap_or("");
        let title = html::text_between(&body, "manga-detail", "</h1>")
        ...
        let chapter_body = if manga_id.is_empty() {
            ...
            fetch_document(
                &format!("{BASE_URL}/ajax-list-chapter?mangaID={manga_id}"),
                CHAPTERS_FIXTURE,
            )
        };
    ...
    fn handle_url(&self, request: Value) -> ExtensionResult<Option<UrlResolveResult>> {
        ...
        if input.starts_with(BASE_URL) {
            return Ok(Some(UrlResolveResult {
                item: input
                    .contains("/manga/")
                    .then(|| parse_details(&fetch_document(input, DETAILS_FIXTURE), Some(normalize_key(input)))),
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(body, "manga-detail", "</h1>")
            ...
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "Manga".to_string())),
        cover: html::attr_after(body, "manga-cover", "src")
...
fn normalize_key(value: &str) -> String {
    if value.starts_with("http://") || value.starts_with("https://") {
        if let Some(index) = value.find(".codes/") {
            return format!("/{}", value[index + 7..].trim_matches('/'));
        }
    }
