...
const SOURCE: MagComi = MagComi;
const BASE_URL: &str = "https://magcomi.com";

struct MagComi;

impl MangaSource for MagComi {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_series(LIST_FIXTURE));
        }
...
fn parse_series(body: &str) -> Paged<CatalogItem> {
    let entries = body
        ...
        .skip(1)
        .filter(|chunk| {
            chunk.contains("SeriesSection_series_list")
                || chunk.contains("jsx-series-thumb")
                || chunk.contains("series")
        })
        .filter_map(|chunk| {
            let href = html::attr_after(chunk, "<a", "href")?;
            if !href.contains("/series/") && !href.contains("/title/") {
                return None;
            }
            let title = html::text_between(chunk, "<h3", "</h3>")
                ...
                .unwrap_or_else(|| url::slug_from_url(&href).unwrap_or_else(|| "MAGCOMI".into()));
            Some(CatalogItem {
...
fn parse_details(body: &str, key: Option<String>) -> CatalogItem {
    ...
    CatalogItem {
        ...
        title: html::text_between(&info, "series-header-title", "</")
            ...
            .map(|value| html::strip_tags(&value))
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| url::slug_from_url(&key).unwrap_or_else(|| "MAGCOMI".into())),
...
<ul class="SeriesSection_series_list__sample"><li><a href="/series/sample"><div class="jsx-series-thumb"><span><noscript><img src="/cover.jpg"></noscript></span></div><h3>Sample MAGCOMI</h3></a></li></ul>
"#;
const SEARCH_FIXTURE: &str = r#"
<ul class="search-series-list"><li class="SearchResultItem_li__sample"><a href="/series/sample"><img src="/cover.jpg"></a><p class="SearchResultItem_series_title__sample">Sample MAGCOMI</p></li></ul>
"#;
const DETAILS_FIXTURE: &str = r#"
<section class="series-information"><div class="series-header"><h1 class="series-header-title">Sample MAGCOMI</h1><h2 class="series-header-author">Sample Author</h2><p class="series-header-description">Sample description.</p><div class="series-header-image-wrapper"><img data-src="/cover.jpg"></div><div class="js-readable-products-pagination" data-aggregate-id="sample"></div></div></section>
"#;
