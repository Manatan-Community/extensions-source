...
const SOURCE: MangaCross = MangaCross;
const BASE_URL: &str = "https://championcross.jp";
const API_URL: &str = "https://championcross.jp/api";
const SEARCH_PAGE_SIZE: u64 = 20;

struct MangaCross;

impl MangaSource for MangaCross {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let page = page(&request);
        Ok(parse_series_list(&fetch_document(
            &format!("{BASE_URL}/category/manga/{page}"),
            LIST_FIXTURE,
        )))
    }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if path == "/ranking/manga" {
            ...
            format!("{BASE_URL}{path}/{}", page(&request))
        };
    ...
    fn pages(&self, request: Value) -> ExtensionResult<Vec<MangaPage>> {
        ...
        Ok(fetch_viewer_pages(&viewer_id))
    }
...
fn parse_series_list(body: &str) -> Paged<CatalogItem> {
    let entries = body
        .split("<div")
        .skip(1)
        .filter(|chunk| chunk.contains("series-list-item"))
        .filter_map(|chunk| {
            ...
            Some(CatalogItem {
                ...
                title: html::text_between(chunk, "series-list-item-h", "</")
                    ...
                    .unwrap_or_else(|| {
                        url::slug_from_url(&key).unwrap_or_else(|| "Champion Cross".into())
                    }),
    ...
    Paged {
        entries,
        has_next_page: body.contains("g-pager-link mode-active") && body.contains("g-pager-link"),
    }
}
...
fn fetch_viewer_pages(viewer_id: &str) -> Vec<MangaPage> {
...
const LIST_FIXTURE: &str = r#"<div class="series-list-item"><a class="series-list-item-link" href="/series/sample"><img class="series-list-item-img" src="/cover.jpg"><div class="series-list-item-h"><span>Sample Champion Cross</span></div></a></div>"#;
const SEARCH_FIXTURE: &str = r#"{"searchResult":{"series":{"total":1,"series":[{"id":"sample","name":"Sample Champion Cross","images":[{"url":"https://img.example.test/cover.jpg"}]}]}}}"#;
const DETAILS_FIXTURE: &str = r#"{"series":{"summary":{"name":"Sample Champion Cross","description":"Sample description.","author":[{"name":"Sample Author"}],"images":[{"url":"https://img.example.test/cover.jpg"}],"tag":[{"name":"Action"}],"isCompleted":false},"episodes":[{"id":"sample","title":"Episode 1","datePublished":1704067200}]}}"#;
