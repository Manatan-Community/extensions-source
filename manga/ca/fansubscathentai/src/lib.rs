...
const SOURCE: HentaiCat = HentaiCat;
const BASE_URL: &str = "https://manga.hentai.cat";
const API_URL: &str = "https://api.hentai.cat";

struct HentaiCat;

impl MangaSource for HentaiCat {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        if request.as_object().is_some_and(|object| object.is_empty()) {
            return Ok(parse_manga_page(MANGA_LIST_FIXTURE, "adult"));
        }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        if query.starts_with(BASE_URL) {
            ...
            return Ok(Paged {
                entries: vec![parse_details(&body, key, "adult")],
                has_next_page: false,
            });
        }
        ...
        append_filters(&mut path, request.get("filters"), true);
        Ok(parse_manga_page(
            &fetch_api_or_fixture(&path, MANGA_LIST_FIXTURE),
            "adult",
        ))
    ...
    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        ...
        Ok(parse_details(&body, key, "adult"))
    }
    ...
    fn handle_url(&self, request: Value) -> ExtensionResult<Option<UrlResolveResult>> {
        ...
        if input.starts_with(BASE_URL) {
            ...
            return Ok(Some(UrlResolveResult {
                item: Some(parse_details(&body, key, "adult")),
...
mod tests {
    ...
    fn parses_api_source() {
        assert_eq!(
            parse_manga_page(MANGA_LIST_FIXTURE, "adult").entries[0].key,
            "sample"
        );
        assert_eq!(
