...
const SOURCE: HasuTl = HasuTl;
const BASE_URL: &str = "https://hasutl.wordpress.com/";

struct HasuTl;

impl NovelSource for HasuTl {
    fn list(&self, _request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let body = fetch_document_or_fixture(&absolute_url("light-novels-activas/"), LIST_FIXTURE);
        Ok(Paged {
    ...
    fn home(&self, request: Value) -> ExtensionResult<Vec<HomeSection<CatalogItem>>> {
        ...
        Ok(vec![HomeSection {
            ...
            title: "Light Novels Activas".to_string(),
...
fn parse_popular(body: &str) -> Vec<CatalogItem> {
    ...
    body.split("wp-block-columns")
        .skip(1)
        .filter_map(|block| {
            let href = html::attr_after(block, "wp-block-button", "href")
                .or_else(|| html::attr_after(block, "<a", "href"))?;
            let key = normalize_key(&href);
            if !seen.insert(key.clone()) {
                return None;
            }
            let title = text_after(block, "wp-block-button")
...
fn parse_search(body: &str) -> Vec<CatalogItem> {
    let mut seen = BTreeSet::new();
    body.split("post-container")
        .skip(1)
        .filter_map(|block| {
            let title = text_after(block, "post-header")?;
            if title.contains("Cap") || title.contains("Vol") || title.contains("Light Novels") {
                return None;
            }
...
fn parse_details(body: &str, key: &str) -> CatalogItem {
    let mut item = catalog_item(
        normalize_key(key),
        text_after(body, "post-header")
            .or_else(|| text_between_tag(body, "h1"))
            .unwrap_or_else(|| url::slug_from_url(key).unwrap_or_else(|| "Novel".to_string())),
        html::attr_after(body, "featured-media", "src")
            .or_else(|| image_from(body))
            .map(|value| absolute_url(&value)),
        true,
    );
    item.description = block_after(body, "post-content")
...
fn parse_chapters(body: &str) -> Vec<NovelChapter> {
    let area =
        block_after(body, "wp-block-media-text__content").unwrap_or_else(|| body.to_string());
    let mut seen = BTreeSet::new();
    area.split("<a")
        .skip(1)
        .filter_map(|chunk| {
...
fn parse_text(body: &str, key: &str) -> NovelText {
    let raw = block_after(body, "post-content").unwrap_or_else(|| body.to_string());
    let normalized = novel::normalize_reader_html(&raw);
    NovelText {
        title: text_after(body, "post-header").or_else(|| text_between_tag(body, "h1")),
...
fn normalize_key(input: &str) -> String {
    input
        ...
        .trim_start_matches("https://hasutl.wordpress.com/")
        .trim_start_matches('/')
        .to_string()
}
...
<div class="wp-block-columns"><img src="/cover.jpg"><div class="wp-block-button"><a href="https://hasutl.wordpress.com/novela/sample/">Sample Novel</a></div></div>
"#;

const SEARCH_FIXTURE: &str = r#"
<div class="post-container"><img src="/cover.jpg"><a href="https://hasutl.wordpress.com/novela/sample/"><h2 class="post-header">Sample Novel</h2></a></div>
"#;

const DETAILS_FIXTURE: &str = r#"
<h1 class="post-header">Sample Novel</h1><div class="featured-media"><img src="/cover.jpg"></div><div class="post-content"><p>Sample summary.</p><div class="wp-block-media-text__content"><a href="https://hasutl.wordpress.com/novela/sample/capitulo-1/">Capitulo 1</a></div></div>
"#;

const TEXT_FIXTURE: &str = r#"
<h1 class="post-header">Capitulo 1</h1><div class="post-content"><p>Sample chapter text.</p></div>
"#;
