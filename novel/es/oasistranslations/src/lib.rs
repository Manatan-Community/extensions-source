...
const SOURCE: OasisTranslations = OasisTranslations;
const BASE_URL: &str = "https://oasistranslations.wordpress.com/";

struct OasisTranslations;

impl NovelSource for OasisTranslations {
    fn list(&self, _request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let body = fetch_document_or_fixture(BASE_URL, LIST_FIXTURE);
        Ok(Paged {
            entries: parse_menu_listing(&body),
            has_next_page: false,
        })
    }
...
fn parse_menu_listing(body: &str) -> Vec<CatalogItem> {
    let menu = body
        .find("menu-item-1819")
    ...
    menu.split("<li")
        ...
        .filter_map(|chunk| {
            ...
            let title = link_text(chunk)?;
            if title.contains("Activas")
                || title.contains("Finalizadas")
                || title.contains("Dropeadas")
                || title.is_empty()
            {
                return None;
            }
            ...
            seen.insert(key.clone())
                .then(|| catalog_item(key, title, image_from(chunk), false))
        })
        .collect()
}
...
fn parse_details(body: &str, key: &str) -> CatalogItem {
    ...
    item.authors = detail_value(&content, "Autor").into_iter().collect();
...
fn parse_text(body: &str, key: &str) -> NovelText {
    ...
    let cleaned = remove_block_containing(&raw, "jp-post-flair");
...
fn key_from_url(input: &str) -> Option<String> {
    input
        .contains("oasistranslations.wordpress.com")
...
fn normalize_key(input: &str) -> String {
    input
        ...
        .trim_start_matches("https://oasistranslations.wordpress.com/")
        .trim_start_matches('/')
        .to_string()
}
...
<li class="menu-item-1819"><ul class="sub-menu"><li><a href="https://oasistranslations.wordpress.com/novela/sample/"><img src="/cover.jpg">Sample Novel</a></li></ul></li>
"#;

const DETAILS_FIXTURE: &str = r#"
<h1 class="entry-title">Sample Novel</h1><div class="entry-content"><p><strong>Autor:</strong> Sample Author<br><strong>Genero:</strong> Fantasia</p><a href="https://oasistranslations.wordpress.com/novela/sample/capitulo-1/">Capitulo 1</a></div>
"#;
