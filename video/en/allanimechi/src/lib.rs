...
const SOURCE: AllAnimeChi = AllAnimeChi;
const SITE_URL: &str = "https://allanime.to";
const BASE_URL: &str = "https://allanime.to";
const API_URL: &str = "https://acapi.allanime.day";
const GRAPHQL_ORIGIN: &str = "https://allanime.to";
const FALLBACK_PLAYER_DOMAIN: &str = "https://blog.allanime.pro";
const PAGE_SIZE: u64 = 30;
const POPULAR_HASH: &str = "31a117653812a2547fd981632e8c99fa8bf8a75c4ef1a77a1567ef1741a7ab9c";
const LATEST_HASH: &str = "e42a4466d984b2c0a2cecae5dd13aa68867f634b16ee0f17b380047d14482406";
const DETAILS_HASH: &str = "bb263f91e5bdd048c1c978f324613aeccdfe2cbc694a419466a31edb58c0cc0b";
const STREAM_HASH: &str = "5e7e17cdd0166af5a2d8f43133d9ce3ce9253d1fdb5160a0cfd515564f98d061";

struct AllAnimeChi;

impl VideoSource for AllAnimeChi {
        ...
        let body = if listing == "latest" {
            post_graphql(
                json!({
                    ...
                    "extensions": persisted_query(LATEST_HASH)
                }),
                SEARCH_FIXTURE,
            )
        } else {
            post_graphql(
                json!({
                    ...
                    "extensions": persisted_query(POPULAR_HASH)
                }),
                POPULAR_FIXTURE,
            )
        };
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = post_graphql(
            json!({
                "variables": {
                    "search": search_payload(query, &request),
                    "limit": PAGE_SIZE,
                    "page": page,
                    "translationType": pref(&request, "preferred_sub", "sub"),
                    "countryOrigin": filter(&request, "origin", "ALL")
                },
                "extensions": persisted_query(LATEST_HASH)
            }),
            SEARCH_FIXTURE,
    ...
    fn episodes(&self, request: Value) -> ExtensionResult<Vec<VideoEpisode>> {
        ...
        let body = post_graphql(
            json!({
                "variables": { "_id": id },
                "extensions": persisted_query(DETAILS_HASH)
            }),
            EPISODES_FIXTURE,
        );
        Ok(parse_episodes(
...
fn post_graphql(body: Value, fixture: &str) -> String {
    ...
    get_or_fixture(&target, fixture, GRAPHQL_ORIGIN)
}
...
fn search_payload(query: &str, request: &Value) -> Value {
    if !query.is_empty() {
        return json!({
            "query": query,
    ...
    search.insert("sortBy".to_string(), json!("Latest_Update"));
    search.insert("allowAdult".to_string(), json!(false));
    search.insert("allowUnknown".to_string(), json!(false));
    search.insert("denyEcchi".to_string(), json!(false));

    for (request_key, api_key) in [("season", "season"), ("release_year", "year")] {
        let value = filter(request, request_key, "all");
        if value != "all" {
            search.insert(api_key.to_string(), json!(value));
        }
    }

    let media_type = filter(request, "type", "all");
    if media_type != "all" {
        search.insert("types".to_string(), json!([media_type]));
    }

    let episode_count = filter(request, "episode_count", "all");
    if episode_count != "all" {
        let mut parts = episode_count.split('-');
        if let Some(start) = parts.next().filter(|value| !value.is_empty()) {
            if let Ok(value) = start.parse::<u64>() {
                search.insert("epRangeStart".to_string(), json!(value));
            }
        }
        if let Some(end) = parts.next().filter(|value| !value.is_empty()) {
            if let Ok(value) = end.parse::<u64>() {
                search.insert("epRangeEnd".to_string(), json!(value));
            }
        }
    }
    ...
    let genres = request
        ...
        .collect::<Vec<_>>();
    if !genres.is_empty() {
        search.insert("genres".to_string(), json!(genres));
    }
