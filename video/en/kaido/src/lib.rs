...
const SOURCE: Kaido = Kaido;
const BASE_URL: &str = "https://kaido.to";
const HOSTERS: [&str; 2] = ["Vidstreaming", "Vidcloud"];
...
impl VideoSource for Kaido {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let body = get_or_fixture(&format!("{BASE_URL}/{path}?page={page}"), LIST_FIXTURE, BASE_URL);
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        for key in ["type", "status", "rated", "score", "season", "language", "sort", "sy", "sm", "sd", "ey", "em", "ed", "genres"] {
        ...
        let query_string = params.into_iter()
            ...
            .collect::<Vec<_>>()
            .join("&");
        let body = get_or_fixture(&format!("{BASE_URL}/{endpoint}?{query_string}"), LIST_FIXTURE, BASE_URL);
        Ok(parse_listing(&body, &request))
    }

    fn details(&self, request: Value) -> ExtensionResult<CatalogItem> {
        let path = request_key(&request, "item").unwrap_or_else(|| "/watch/sample-anime-1".to_string());
    ...
    fn episodes(&self, request: Value) -> ExtensionResult<Vec<VideoEpisode>> {
        ...
        let body = ajax_html(
            &format!("{BASE_URL}/ajax/episode/list/{id}"),
            EPISODES_FIXTURE,
            &absolute_url(&path),
        );
    ...
    fn hosters(&self, request: Value) -> ExtensionResult<Vec<VideoHoster>> {
        let episode = request_key(&request, "episode").unwrap_or_else(|| "/watch/sample-anime-1?ep=1".to_string());
        let id = episode.split("?ep=").nth(1).and_then(|tail| tail.split(['&', '#']).next()).unwrap_or("1");
        let body = ajax_html(
            &format!("{BASE_URL}/ajax/episode/servers?episodeId={id}"),
            HOSTERS_FIXTURE,
    ...
    fn resolve_hoster(&self, request: Value) -> ExtensionResult<Vec<VideoStream>> {
        let Some(key) = request_raw_key(&request, "hoster") else { return Ok(Vec::new()) };
        ...
        let body = get_or_fixture(
            &format!("{BASE_URL}/ajax/episode/sources?id={server_id}"),
            SOURCE_FIXTURE,
            referer,
        );
        let link = serde_json::from_str::<SourceLink>(&body).ok().and_then(|res| res.link).unwrap_or_default();
    ...
    fn home(&self, request: Value) -> ExtensionResult<Vec<HomeSection<CatalogItem>>> {
        ...
        Ok(vec![
            HomeSection { id: "popular".to_string(), title: "Most Popular".to_string(), style: Some(HomeSectionStyle::Featured), entries: popular.entries, has_more: popular.has_next_page, ..HomeSection::default() },
            HomeSection { id: "latest".to_string(), title: "Top Airing".to_string(), entries: latest.entries, has_more: latest.has_next_page, ..HomeSection::default() },
        ])
    }
...
fn client(referer: &str) -> HttpClient {
    HttpClient::browser()
        ...
        .with_header("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8")
...
fn ajax_html(target: &str, fixture: &str, referer: &str) -> String {
    let body = client(referer).get(target).xhr().referer(referer).send_text().unwrap_or_else(|_| fixture.to_string());
    serde_json::from_str::<HtmlResponse>(&body).map(|response| response.html).unwrap_or(body)
}
...
fn parse_listing(body: &str, request: &Value) -> Paged<CatalogItem> {
    Paged {
        entries: body.split("flw-item").skip(1).filter_map(|chunk| parse_card(chunk, request)).collect(),
        has_next_page: body.contains("page-item") && body.contains("title=\"Next\""),
    }
}

fn parse_card(chunk: &str, request: &Value) -> Option<CatalogItem> {
    let href = html::attr_after(chunk, "film-detail", "href").or_else(|| html::attr_after(chunk, "<a", "href"))?;
    let path = path_key(&href);
    let use_english = preference(request, "preferred_title_lang").map(|v| v == "English").unwrap_or(false);
    let title = if use_english {
        html::attr_after(chunk, "film-detail", "title").or_else(|| html::attr_after(chunk, "<a", "title"))
    } else {
        html::attr_after(chunk, "film-detail", "data-jname").or_else(|| html::attr_after(chunk, "<a", "data-jname"))
    }.filter(|value| !value.trim().is_empty()).unwrap_or_else(|| title_from_path(&path));
    Some(CatalogItem {
        key: path.clone(),
        title,
...
fn parse_details(body: &str, path: &str) -> Option<CatalogItem> {
    ...
    Some(CatalogItem {
        key: path_key(path),
        title,
        cover: html::attr_after(body, "anisc-poster", "src").or_else(|| html::attr_after(body, "<img", "src")).map(|image| absolute_url(&image)),
        url: Some(absolute_url(path)),
...
fn info_value(info: &str, tag: &str) -> Option<String> {
    let block = info.split(tag).nth(1)?.split("</div>").next().unwrap_or_default();
    let value = html::strip_tags(block).trim_start_matches(tag).trim().to_string();
    (!value.is_empty()).then_some(value)
}

fn info_list(info: &str, tag: &str) -> Vec<String> {
    info.split(tag).nth(1).unwrap_or_default()
        .split("</div>").next().unwrap_or_default()
        .split("<a").skip(1)
...
fn parse_episodes(body: &str, request: &Value) -> Vec<VideoEpisode> {
    ...
    body.split("ep-item").skip(1).filter_map(|chunk| {
        let href = html::attr_after(chunk, "<a", "href")?;
        let number = html::attr(chunk, "data-number").and_then(|value| value.parse::<f32>().ok()).unwrap_or(1.0);
        ...
        Some(VideoEpisode {
            ...
            language: Some("en".to_string()),
            is_filler: filler,
            labels: if filler { vec!["Filler Episode".to_string()] } else { Vec::new() },
...
fn parse_hosters(body: &str, episode_path: &str, request: &Value) -> Vec<VideoHoster> {
    ...
    for (type_id, type_name) in TYPES {
        ...
        for chunk in block.split("div class=\"item").skip(1) {
            let Some(server_id) = html::attr(chunk, "data-id") else { continue };
            let media_type = html::attr(chunk, "data-type").unwrap_or_else(|| type_name.to_lowercase());
            let name = html::strip_tags(chunk.split("</div>").next().unwrap_or_default());
            if name.is_empty() || !enabled_hosters.iter().any(|hoster| hoster.eq_ignore_ascii_case(&name)) {
                continue;
            }
...
fn resolve_embed_streams(embed: &str, media_type: &str, name: &str, request: &Value) -> Vec<VideoStream> {
    if embed.is_empty() {
        return Vec::new();
    }
    ...
    if let Some(streams) = resolve_megacloud(embed, media_type, name, request).filter(|streams| !streams.is_empty()) {
        return streams;
    }
        ...
        .or_else(|| html::text_between(&body, "file: '", "'"))
    {
        ...
        return vec![media_stream(&src, media_type, name, "direct", embed, Vec::new(), request)];
    }
    vec![external_stream(embed, media_type, name, request)]
}

fn resolve_megacloud(embed: &str, media_type: &str, name: &str, request: &Value) -> Option<Vec<VideoStream>> {
    ...
    let nonce_body = client(&server_url).get(embed).xhr().referer(&server_url).send_text().ok()?;
    let nonce = find_nonce(&nonce_body)?;
    let sources_url = format!("{server_url}/embed-2/v3/e-1/getSources?id={id}&_k={nonce}");
    let body = client(&server_url).get(&sources_url).xhr().referer(&server_url).send_text().ok()?;
    let response = serde_json::from_str::<Value>(&body).ok()?;
    let tracks = parse_tracks(&response, &server_url);
    let encrypted = response.get("encrypted").and_then(Value::as_bool).unwrap_or(true);
    let Some(sources) = response.get("sources") else { return Some(Vec::new()) };
        ...
        for item in items {
            if let Some(file) = item.get("file").and_then(Value::as_str) {
                if file.contains(".m3u8") {
                    out.extend(parse_hls(file, media_type, name, &server_url, tracks.clone(), request));
                } else {
                    out.push(media_stream(file, media_type, name, "direct", &server_url, tracks.clone(), request));
                }
            }
        }
    }
...
fn find_nonce(body: &str) -> Option<String> {
    ...
    let tokens = body.split(|ch: char| !ch.is_ascii_alphanumeric()).filter(|token| token.len() == 16).take(3).collect::<Vec<_>>();
    (tokens.len() == 3).then(|| tokens.join(""))
}

fn parse_tracks(response: &Value, referer: &str) -> Vec<SubtitleTrack> {
    response.get("tracks").and_then(Value::as_array).into_iter().flatten()
        .filter(|track| track.get("kind").and_then(Value::as_str).map(|kind| kind == "captions" || kind == "subtitles").unwrap_or(false))
        .filter_map(|track| {
            let file = track.get("file")?.as_str()?;
            let label = track.get("label").and_then(Value::as_str).unwrap_or("Subtitle");
            Some(SubtitleTrack {
...
fn parse_hls(target: &str, media_type: &str, name: &str, referer: &str, subtitles: Vec<SubtitleTrack>, request: &Value) -> Vec<VideoStream> {
    let body = client(referer).get(target).referer(referer).send_text().unwrap_or_default();
    if !body.contains("#EXT-X-STREAM-INF") {
        return vec![media_stream(target, media_type, name, "auto", referer, subtitles, request)];
    }
    body.split("#EXT-X-STREAM-INF:").skip(1).filter_map(|block| {
        ...
        Some(media_stream(&stream_url, media_type, name, &quality, referer, subtitles.clone(), request))
    }).collect()
}

fn media_stream(stream_url: &str, media_type: &str, name: &str, quality: &str, referer: &str, subtitles: Vec<SubtitleTrack>, request: &Value) -> VideoStream {
...
fn sort_streams(streams: &mut [VideoStream], request: &Value) {
    streams.sort_by_key(|stream| {
        ...
        let quality_score = quality.chars().filter(char::is_ascii_digit).collect::<String>().parse::<i32>().unwrap_or(0);
        let preferred = i32::from(is_preferred(quality, stream.name.as_deref().unwrap_or_default(), stream.name.as_deref().unwrap_or_default(), request));
...
fn is_preferred(quality: &str, server: &str, media_type: &str, request: &Value) -> bool {
    let pref_quality = preference(request, "preferred_quality").unwrap_or_else(|| "1080".to_string());
    let pref_server = preference(request, "preferred_server").unwrap_or_else(|| "Vidstreaming".to_string());
    let pref_type = preference(request, "preferred_type").unwrap_or_else(|| "Sub".to_string());
    quality.contains(&pref_quality)
        || (server.to_lowercase().contains(&pref_server.to_lowercase()) && media_type.to_lowercase().contains(&pref_type.to_lowercase()))
}

fn enabled_hosters(request: &Value) -> Vec<String> {
    request.get("preferences").and_then(|p| p.get("hoster_selection")).and_then(Value::as_array)
...
fn enabled_types(request: &Value) -> Vec<String> {
    request.get("preferences").and_then(|p| p.get("type_selection")).and_then(Value::as_array)
        .map(|values| values.iter().filter_map(Value::as_str).map(ToString::to_string).collect())
...
fn path_key(input: &str) -> String {
    ...
    let path = if path.starts_with("/watch/") { path.split('&').next().unwrap_or(path) } else { path.split('?').next().unwrap_or(path) };
    format!("/{}", path.trim_matches('/'))
}

fn absolute_url(input: &str) -> String {
    if input.starts_with("http") { input.to_string() } else { url::join_url(BASE_URL, input) }
}
...
fn title_from_path(input: &str) -> String {
    input.split('?').next().unwrap_or(input).trim_matches('/').rsplit('/').next().unwrap_or("Kaido")
        .rsplit_once('-').map(|(name, _)| name).unwrap_or(input).replace('-', " ")
}
...
fn language_code(label: &str) -> Option<String> {
    label.to_lowercase().contains("english").then(|| "en".to_string())
}

fn preference(request: &Value, key: &str) -> Option<String> {
    request.get("preferences").and_then(|p| p.get(key)).or_else(|| request.get(key)).and_then(Value::as_str).map(ToString::to_string)
}

fn preference_bool(request: &Value, key: &str, default: bool) -> bool {
    request.get("preferences").and_then(|p| p.get(key)).or_else(|| request.get(key))
        .and_then(|value| value.as_bool().or_else(|| value.as_str().and_then(|text| text.parse().ok())))
        .unwrap_or(default)
}
...
const LIST_FIXTURE: &str = r#"<div class="flw-item"><div class="film-poster"><img data-src="/poster.jpg"></div><div class="film-detail"><a href="/watch/sample-anime-1" title="Sample Anime" data-jname="Sample Anime">Sample Anime</a></div></div>"#;
const DETAILS_FIXTURE: &str = r#"<h2 class="film-name">Sample Anime</h2><div class="anisc-poster"><img src="/poster.jpg"></div><div class="anisc-info"><div class="item item-title">Overview: <span class="text">Sample overview.</span></div><div class="item item-title">Status: <span class="name">Currently Airing</span></div><div class="item-list">Genres: <a>Action</a></div><div class="item item-title">Studios: <span class="name">Sample Studio</span></div></div>"#;
const EPISODES_FIXTURE: &str = r#"<a class="ep-item" href="/watch/sample-anime-1?ep=1" data-number="1" title="First Episode"></a>"#;
const HOSTERS_FIXTURE: &str = r#"<div class="servers-sub"><div class="item" data-id="1" data-type="sub">Vidstreaming</div><div class="item" data-id="2" data-type="sub">Vidcloud</div></div>"#;
