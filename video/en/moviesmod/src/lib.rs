...
const SOURCE: MoviesMod = MoviesMod;
const DEFAULT_BASE_URL: &str = "https://moviesmod.red";

struct MoviesMod;

impl VideoSource for MoviesMod {
...
fn parse_listing(body: &str, base: &str) -> Paged<CatalogItem> {
    ...
    Paged {
        entries: select_all(&doc, "div#content_box div.post-cards > article")
            .filter_map(|element| card_item(element, base))
            .collect(),
        has_next_page: select_all(&doc, "#content_box > nav > div > a.next.page-numbers")
            .next()
            .is_some(),
    }
}
...
fn card_item(element: ElementRef<'_>, base: &str) -> Option<CatalogItem> {
    ...
    let title = attr(&element, "a", "title")
    ...
    Some(CatalogItem {
        key: key.clone(),
        title,
        cover: attr(&element, "div.featured-thumbnail > img", "src")
...
fn parse_details(body: &str, path: &str, base: &str) -> Option<CatalogItem> {
    ...
    Some(CatalogItem {
        ...
        authors: select_text(
            &doc,
            "div.entry-content > div.thecontent > div.imdbwp > div.imdbwp__content > div.imdbwp__footer > span",
        )
        .into_iter()
        .collect(),
        description: select_text(
            &doc,
            "div.entry-content > div.thecontent > div.imdbwp > div.imdbwp__content > div.imdbwp__teaser",
        )
        .or_else(|| select_text(&doc, "div.entry-content > div.thecontent")),
        status: ItemStatus::Unknown,
        url: Some(absolute_url(path, base)),
        language: Some("en".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str, item_path: &str, base: &str) -> Vec<VideoEpisode> {
    ...
    let rows = select_all(
        &doc,
        "p:has(a.maxbutton-episode-links), p:has(a.maxbutton-download-links)",
    )
    .collect::<Vec<_>>();
    if rows.is_empty() {
        return Vec::new();
    ...
    let is_series = rows
        .first()
        .and_then(|row| text(row, "a"))
        .is_some_and(|value| value.eq_ignore_ascii_case("Episode Links"));
    ...
    for row in rows {
        ...
        let default_name = if is_series {
            season_regex
                .find(&prev_text)
                .map(|value| value.as_str().trim().to_string())
                .unwrap_or_else(|| "Season 1".to_string())
        } else {
            clean_title(prev_text.lines().next().unwrap_or("Movie"))
        };
        let Some(episode_page_url) = attr(&row, "a[href]", "href") else {
            continue;
        };
        let Some(child_url) = extract_child_url(&episode_page_url) else {
            continue;
        };
        let episode_page = get_or_fixture(&child_url, EPISODE_LINKS_FIXTURE, base);
        let episode_doc = Html::parse_document(&episode_page);
        for (index, link) in
            select_all(&episode_doc, "div.timed-content-client_show_0_5_0 a").enumerate()
        {
...
fn extract_child_url(main_url: &str) -> Option<String> {
    let encoded = query_param(main_url, "url")?;
...
const BOUNDARY: &str = "----manatanmoviesmodboundary";
...
<div id="content_box">
  <div class="post-cards">
    <article>
      <a href="/sample-movie/" title="Download Sample Movie">Sample Movie</a>
      <div class="featured-thumbnail"><img src="/sample.jpg"></div>
    </article>
  </div>
</div>
"#;
...
<div class="entry-content"><div class="thecontent">
  <div class="imdbwp"><div class="imdbwp__content">
    <div class="imdbwp__teaser">Sample description.</div>
    <div class="imdbwp__footer"><span>Sample Author</span></div>
  </div></div>
</div></div>
"#;

const EPISODE_LINKS_FIXTURE: &str = r#"
<div class="timed-content-client_show_0_5_0"></div>
"#;
