...
const SOURCE: AniGo = AniGo;
const BASE_URL: &str = "https://anigo.to";
const HOSTERS: [&str; 2] = ["Server 1", "Server 2"];
struct AniGo;

impl VideoSource for AniGo {
    ...
    fn episodes(&self, request: Value) -> ExtensionResult<Vec<VideoEpisode>> {
        ...
        let id = anime_id(&details)
            .unwrap_or_else(|| path.rsplit('-').next().unwrap_or("1").to_string());
        let body = get_json(
            &format!(
                "{BASE_URL}/api/v1/titles/{id}/episodes?_={}",
                enc_endpoint(&id)
            ),
            EPISODES_JSON_FIXTURE,
            &absolute_url(&path),
        );
        let mut episodes = parse_anigo_episodes(&body, &request);
        episodes.reverse();
        Ok(episodes)
    }
    ...
    fn hosters(&self, request: Value) -> ExtensionResult<Vec<VideoHoster>> {
        ...
        let body = get_json(
            &format!("{BASE_URL}/api/v1/eptokens/{id}?_={}", enc_endpoint(id)),
            HOSTERS_JSON_FIXTURE,
            &absolute_url(&episode),
        );
        Ok(parse_anigo_hosters(&body, &episode, &request))
    }
    ...
    fn resolve_hoster(&self, request: Value) -> ExtensionResult<Vec<VideoStream>> {
        ...
        let body = get_json(
            &format!(
                "{BASE_URL}/api/v1/links/{server_id}?_={}",
...
fn get_json(target: &str, fixture: &str, referer: &str) -> String {
    client(referer)
        .get(target)
        .xhr()
...
fn parse_listing(body: &str, request: &Value) -> Paged<CatalogItem> {
    let chunks = if body.contains("aniCard") || body.contains(" class=\"unit") {
        body.split("class=\"unit")
            .skip(1)
            .filter(|chunk| chunk.contains("poster") || chunk.contains("title"))
            .collect::<Vec<_>>()
    } else if body.contains("aitem-wrapper") || body.contains(" aitem") {
        body.split("aitem")
            .skip(1)
            .filter(|chunk| chunk.contains("poster") || chunk.contains("title"))
            .collect::<Vec<_>>()
...
fn parse_details(body: &str, path: &str) -> Option<CatalogItem> {
    ...
    let title = html::text_between(body, "div title", "</div>")
        .or_else(|| html::text_between(body, "h1 title", "</h1>"))
...
fn anime_id(body: &str) -> Option<String> {
    html::attr_after(body, "data-id", "data-id")
        .or_else(|| html::attr(body, "data-id"))
        .or_else(|| {
            let x_data = html::attr_after(body, "playZone", "x-data")
                .or_else(|| html::attr_after(body, "x-data", "x-data"))?;
            x_data
                .split("id:")
                .nth(1)?
                .split('\'')
...
fn parse_anigo_episodes(body: &str, request: &Value) -> Vec<VideoEpisode> {
    let mark_fillers = preference_bool(request, "mark_fillers", true);
    let payload = serde_json::from_str::<AniGoEpisodesResponse>(body).unwrap_or_default();
    payload
        .result
        .ranged_episodes
        .into_iter()
        .flat_map(|range| range.episodes)
        .map(|episode| {
            let number = episode.number as f32;
            let detail = episode
                .detail_name
                .filter(|name| !name.trim().is_empty())
                .unwrap_or_else(|| episode.name.clone());
            let filler = mark_fillers && episode.is_filler == 1;
            VideoEpisode {
                key: episode.token,
                title: Some(if detail.trim().is_empty() {
                    format!("Episode {}", display_number(number))
                } else {
                    format!("Episode {}: {detail}", display_number(number))
                }),
...
fn parse_anigo_hosters(body: &str, episode_path: &str, request: &Value) -> Vec<VideoHoster> {
    ...
    let payload = serde_json::from_str::<AniGoEpTokenResponse>(body).unwrap_or_default();
    payload
        .result
        .into_iter()
        .filter(|token| enabled_types.iter().any(|value| value == &token.lang))
        .flat_map(|token| {
            let media_type = type_name(&token.lang).to_string();
            let episode_url = episode_url.clone();
            let enabled_hosters = enabled_hosters.clone();
            token.links.into_iter().filter_map(move |link| {
                if !enabled_hosters
                    .iter()
                    .any(|hoster| hoster.eq_ignore_ascii_case(&link.server_title))
                {
                    return None;
                }
                Some(VideoHoster {
                    key: format!(
                        "{}|{}|{}|{}",
                        link.id, media_type, link.server_title, episode_url
                    ),
                    name: format!("{} - {}", link.server_title, media_type),
...
struct AniGoEpisodesResponse {
    #[serde(default)]
    result: AniGoEpisodesResult,
}

#[derive(Default, Deserialize)]
struct AniGoEpisodesResult {
    #[serde(default, rename = "rangedEpisodes")]
    ranged_episodes: Vec<AniGoEpisodeRange>,
}

#[derive(Default, Deserialize)]
struct AniGoEpisodeRange {
    #[serde(default)]
    episodes: Vec<AniGoEpisode>,
}
...
struct AniGoEpisode {
    ...
    #[serde(default)]
    name: String,
    #[serde(default, rename = "detail_name")]
...
#[derive(Default, Deserialize)]
struct AniGoEpTokenResponse {
    #[serde(default)]
    result: Vec<AniGoEpToken>,
}
...
struct AniGoEpToken {
    ...
    lang: String,
    #[serde(default)]
    links: Vec<AniGoServerLink>,
}

#[derive(Default, Deserialize)]
struct AniGoServerLink {
    id: String,
...
<div id="main-entity" data-id="sample-anime-id">
  <div class="title" x-data="JTitle(`Sample Anime`)">Sample Anime</div>
  <div class="playZone" x-data="{ id: 'sample-anime-id' }"></div>
...
"#;
const EPISODES_JSON_FIXTURE: &str = r#"{"status":"ok","result":{"rangedEpisodes":[{"episodes":[{"number":1,"name":"First Episode","detail_name":"First Episode","langs":3,"is_filler":0,"token":"sample-token"}]}]}}"#;
const HOSTERS_JSON_FIXTURE: &str = r#"{"status":"ok","result":[{"id":"sample-token","lang":"softsub","number":1,"links":[{"id":"1","server_id":1,"server_title":"Server 1"},{"id":"2","server_id":2,"server_title":"Server 2"}]}]}"#;
