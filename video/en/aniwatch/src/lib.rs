...
const SOURCE: AniWatch = AniWatch;
const BASE_URL: &str = "https://aniwatchtv.to";
const AJAX_ROUTE: &str = "/v2";
const HOSTERS: [&str; 2] = ["VidSrc", "MegaCloud"];
...
fn is_preferred(quality: &str, server: &str, media_type: &str, request: &Value) -> bool {
    ...
    let pref_server =
        preference(request, "preferred_server").unwrap_or_else(|| "VidSrc".to_string());
...
<div class="servers-sub"><div class="item" data-id="1" data-type="sub">VidSrc</div><div class="item" data-id="2" data-type="sub">MegaCloud</div></div>
"#;
