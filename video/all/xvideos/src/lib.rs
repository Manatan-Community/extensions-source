...
const SOURCE: Xvideos = Xvideos;
const BASE_URL: &str = "https://www.xvideos.com";

struct Xvideos;

impl VideoSource for Xvideos {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let body = client()
            .get(format!("{BASE_URL}/new/{}", page(&request)))
            .browser_document()
            .send_text()
            .unwrap_or_default();
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if !query.is_empty() {
            ...
            format!("{BASE_URL}/new/{}", page(&request))
        };
        let body = client()
            .get(target)
            .browser_document()
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    ...
    Paged {
        entries,
        has_next_page: body.contains("next-page"),
    }
}
...
fn parse_card(block: &str) -> Option<CatalogItem> {
    ...
    let title = html::text_between(block, "title", "</p>")
