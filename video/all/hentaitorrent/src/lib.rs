...
use manatan_extension::{
    CatalogItem, ItemStatus, Paged, TorrentInfo, UrlResolveResult, VideoEpisode, VideoStream,
    VideoStreamKind,
...
const SOURCE: HentaiTorrent = HentaiTorrent;
const BASE_URL: &str = "https://www.hentaitorrents.com";

struct HentaiTorrent;

impl VideoSource for HentaiTorrent {
    ...
    fn episodes(&self, request: Value) -> ExtensionResult<Vec<VideoEpisode>> {
        ...
        let download_body = client()
            ...
            .send_text()
            .unwrap_or_default();
        let torrent_path = html::attr_after(&download_body, "download-button", "href")
            .ok_or_else(|| error("No torrent file found"))?;
        let torrent_url = url::join_url(BASE_URL, &torrent_path);
        let torrent_bytes = fetch_bytes(&torrent_url)?;
        let torrent = parse_torrent(&torrent_bytes, &torrent_url)?;
...
fn fetch_details(key: &str) -> CatalogItem {
    ...
    let title = html::attr_after(&body, "<meta property=\"og:title\"", "content")
        ...
        .unwrap_or_else(|| key.clone());
    let description = html::text_between(&body, "article-content", "</div>")
        .map(|text| html::strip_tags(&text))
        .filter(|text| !text.is_empty());
    CatalogItem {
        key: key.clone(),
...
fn parse_torrent(bytes: &[u8], _torrent_url: &str) -> ExtensionResult<ParsedTorrent> {
    let (info_start, info_end) = info_slice(bytes).ok_or_else(|| error("Torrent missing info dictionary"))?;
...
fn parse_value(bytes: &[u8], pos: usize) -> Option<(BValue, usize)> {
    match *bytes.get(pos)? {
        b'i' => {
            ...
            let value = std::str::from_utf8(&bytes[pos + 1..end]).ok()?.parse().ok()?;
...
fn parse_bytes_raw(bytes: &[u8], pos: usize) -> Option<(&[u8], usize)> {
    ...
    let len = std::str::from_utf8(&bytes[pos..colon]).ok()?.parse::<usize>().ok()?;
    let start = colon + 1;
...
fn dict_value<'a>(dict: &'a [(Vec<u8>, BValue)], key: &[u8]) -> Option<&'a BValue> {
    dict.iter().find(|(candidate, _)| candidate == key).map(|(_, value)| value)
}
...
fn id_from_url(input: &str) -> Option<String> {
    ...
    if input.starts_with("http") && !input.contains("hentaitorrents.com") {
        return None;
    }
    input
        .split("/anime/")
        ...
        .filter(|value| !value.is_empty() && value != "hentaitorrents.com")
}
...
fn is_video_file(path: &str) -> bool {
    matches!(
        ...
        "mp4" | "mov" | "avi" | "wmv" | "mkv" | "flv" | "webm" | "mpeg" | "mpg" | "mts" | "vob" | "ts"
    )
}
