...
const SOURCE: AnimeFenix = AnimeFenix;
const BASE_URL: &str = "https://animefenix2.tv";

struct AnimeFenix;

impl VideoSource for AnimeFenix {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let page = page(&request);
        let body = get_or_fixture(
            &format!("{BASE_URL}/directorio/anime?p={page}&estado=2"),
            LIST_FIXTURE,
            BASE_URL,
        );
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if !query.is_empty() {
            format!(
                "{BASE_URL}/directorio/anime?q={}&p={page}",
            ...
            if params.is_empty() {
                ...
                format!("{BASE_URL}/directorio/anime?{params}&p={page}")
            }
        };
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&document, "h1.text-4xl, h1")
            .unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&document, "#anime_image", "data-src")
            .or_else(|| select_attr_doc(&document, "#anime_image", "src"))
            .map(|src| absolute_url(&src)),
        url: Some(absolute_url(path)),
        description: select_text_doc(&document, ".mb-6 p.text-gray-300, .mb-6 p"),
        tags: select_texts_doc(&document, ".flex-wrap a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str) -> Vec<VideoEpisode> {
    let document = Html::parse_document(body);
    let mut episodes = select_all(&document, ".divide-y li > a, a[href*='/ver/']")
        .into_iter()
        .filter_map(|el| {
            let href = attr(&el, "href");
            ...
            let title = select_text(el, ".font-semibold")
