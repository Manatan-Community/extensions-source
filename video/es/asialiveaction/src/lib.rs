...
const SOURCE: AsiaLiveAction = AsiaLiveAction;
const BASE_URL: &str = "https://asialiveaction.com";

struct AsiaLiveAction;

impl VideoSource for AsiaLiveAction {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let page = page(&request);
        let target = if page == 1 { format!("{BASE_URL}/navegacion/") } else { format!("{BASE_URL}/navegacion/?pagina={page}") };
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if !query.is_empty() {
            format!(
                "{BASE_URL}/?s={}&pagina={page}",
                url::query_escape(query)
            )
        } else {
            let params = filter_params(&request);
            if params.is_empty() {
                format!("{BASE_URL}/navegacion/")
            } else {
                format!("{BASE_URL}/navegacion/?{params}&pagina={page}")
            }
        };
        let body = get_or_fixture(&target, LIST_FIXTURE, BASE_URL);
        Ok(parse_listing(&body))
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    ...
    let entries = select_all(&document, "ul.navegacion-grid li")
        .into_iter()
...
fn card_from_entry(el: ElementRef<'_>) -> Option<CatalogItem> {
    let href = select_attr(el, "a[href]", "href")?;
    Some(CatalogItem {
        key: path_key(&href),
        title: select_text(el, "h5, h4, h3.Title, a.Title, span.Title")
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&document, "h2.Title, h1.Title, h1")
            .unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&document, "figure img, img", "src")
            .or_else(|| background_url(&select_attr_doc(&document, "div.Poster", "style").unwrap_or_default()))
            .map(|src| absolute_url(&src)),
        url: Some(absolute_url(path)),
        description: select_text_doc(&document, "section article, div.Description"),
        tags: select_texts_doc(&document, "footer a.tag, .categorias a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str) -> Vec<VideoEpisode> {
    let document = Html::parse_document(body);
    let mut episodes = select_all(&document, "div.lista-episodios div.episodio-unico a, a[href*='/ver/']")
        .into_iter()
        .filter_map(|el| {
            ...
            let title = select_text(el, "span.episodio-serie, b.numero-episodio, .font-semibold")
                .or_else(|| Some(element_text(el)))
                .unwrap_or_else(|| title_from_path(&href));
            let number = Regex::new(r#"(?i)episodio\s*([0-9]+(?:\.[0-9]+)?)"#)
                .unwrap()
                .captures(&title)
...
fn media_urls(body: &str) -> Vec<String> {
    ...
    if let Some(raw) = Regex::new(r#"(?s)var\s+allVideos\s*=\s*(\{.*?\});"#)
        .ok()
        .and_then(|re| re.captures(body))
        .and_then(|cap| cap.get(1).map(|m| m.as_str().to_string()))
    {
        for cap in Regex::new(r#""(https?://[^"]+)""#).unwrap().captures_iter(&raw) {
            if let Some(url) = cap.get(1) {
                urls.push(clean_embed_url(url.as_str()));
            }
        }
    }
    for cap in Regex::new(r#"https?://[^\s"'<>\\]+"#).unwrap().captures_iter(body) {
        urls.push(clean_embed_url(cap.get(0).unwrap().as_str()));
    }
    urls.sort();
    urls.dedup();
    urls.retain(|url| {
...
fn clean_embed_url(input: &str) -> String {
    input
        .trim()
        .trim_matches(['"', '\''])
        .replace("\\/", "/")
...
fn background_url(style: &str) -> Option<String> {
    Regex::new(r#"url\((['"]?)([^'")]+)['"]?\)"#)
        .ok()?
        .captures(style)?
        .get(2)
