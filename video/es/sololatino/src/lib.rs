...
const SOURCE: SoloLatino = SoloLatino;
const BASE_URL: &str = "https://sololatino.net";
const AJAX_URL: &str = "https://sololatino.net/wp-admin/admin-ajax.php";
const AES_KEY: &str = "Ak7qrvvH4WKYxV2OgaeHAEg2a5eh16vE";

struct SoloLatino;

impl VideoSource for SoloLatino {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        Ok(parse_listing(&fetch(
            &format!("{BASE_URL}/{path}/page/{}", page(&request)),
            LIST_FIXTURE,
            BASE_URL,
        )))
    }
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        } else if let Some(path) = filter_path(&request) {
            format!("{BASE_URL}{path}/page/{page}")
        } else {
            format!("{BASE_URL}/tendencias/page/{page}")
        };
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    ...
    Paged {
        entries: doc
            ...
            .collect(),
        has_next_page: doc
            .select(&selector("div.pagMovidy a, .pagination a"))
            .next()
            .is_some(),
    }
}
...
fn card(el: ElementRef<'_>) -> Option<CatalogItem> {
    let href = select_attr(el, "a[href]", "href").or_else(|| attr(&el, "href"))?;
    let image = el.select(&selector("img")).next();
    let title = image
        .as_ref()
    ...
    Some(CatalogItem {
        key: path_key(&href),
        title,
        cover: image.and_then(|img| image_url(img)),
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&doc, "div.sheader div.data h1, h1")
            .unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(
            &doc,
            "div.sheader div.poster img, div.poster img, meta[property='og:image']",
            "src",
        )
        ...
        description: select_text_doc(
            &doc,
            "#single div.wp-content, div.wp-content, div.description",
        ),
        tags: select_texts_doc(&doc, "div.sgeneros a, a[href*='/genres/']"),
...
fn parse_episodes(body: &str, item_path: &str) -> Vec<VideoEpisode> {
    let doc = Html::parse_document(body);
    let seasons = doc
        .select(&selector("div#seasons div.se-c"))
        .collect::<Vec<_>>();
    if seasons.is_empty() {
    ...
    for season in seasons {
        let season_name = attr(&season, "data-season").unwrap_or_else(|| "1".to_string());
        for element in season.select(&selector("ul.episodios li")) {
            let ep_num = select_text(element, "div.numerando")
                .and_then(|text| number_re.find(&text).map(|m| m.as_str().to_string()))
                .unwrap_or_else(|| "0".to_string());
            let href = select_attr(element, "a[href]", "href").unwrap_or_default();
            let episode_name =
                select_text(element, "div.epst").unwrap_or_else(|| "Sin titulo".to_string());
            let key = path_key(&href);
            episodes.push(VideoEpisode {
                key: key.clone(),
                title: Some(format!(
                    "T{season_name} - Episodio {ep_num}: {episode_name}"
                )),
                episode_number: ep_num.parse::<f32>().ok(),
                season_number: season_name.parse::<f32>().ok(),
...
fn parse_hosters(body: &str, referer: &str) -> Vec<VideoHoster> {
    ...
    let player_re = Regex::new(
        r#"data-type=["'](.+?)["']\s+data-post=["'](.+?)["']\s+data-nume=["'](.+?)["']"#,
    )
    .unwrap();
    for cap in player_re.captures_iter(body) {
        let player_type = cap.get(1).map(|m| m.as_str()).unwrap_or_default();
        let post = cap.get(2).map(|m| m.as_str()).unwrap_or_default();
        let nume = cap.get(3).map(|m| m.as_str()).unwrap_or_default();
        let response = post_ajax(
            &[
        ...
        if let Some(iframe) = iframe_src(&response) {
            let page = fetch(&absolute_url(&iframe), "", referer);
            pairs.extend(parse_link_page(&page));
        }
    }
    if pairs.is_empty() {
        if let Some(iframe) = Regex::new(r#"pframe"><iframe class="[^"]+" src="([^"]+)""#)
            .unwrap()
            .captures(body)
            .and_then(|cap| cap.get(1).map(|m| m.as_str().to_string()))
        {
            let page = fetch(&absolute_url(&iframe), "", referer);
            pairs.extend(parse_link_page(&page));
            if pairs.is_empty() && iframe.contains("xyz") {
                pairs.push((iframe, "unknown".to_string()));
            }
        }
    }
    pairs
        ...
        .map(|(embed, lang)| {
            ...
            VideoHoster {
                key: format!("{name}|{}|{lang}|{referer}", absolute_url(&embed)),
...
fn parse_link_page(body: &str) -> Vec<(String, String)> {
    let mut links = parse_data_links(body);
    links.extend(
        parse_old_links(body)
            .into_iter()
            .map(|link| (link, "unknown".to_string())),
    );
    links
}
...
fn parse_old_links(body: &str) -> Vec<String> {
    let mut out = Vec::new();
    let decoded_re = Regex::new(r#"\.php\?link=(.+?)&servidor="#).unwrap();
    for cap in Regex::new(r#"onclick=["']([^"']+)["']"#)
        .unwrap()
        .captures_iter(body)
    {
        let onclick = cap.get(1).map(|m| m.as_str()).unwrap_or_default();
        if let Some(encoded) = decoded_re
            .captures(onclick)
            .and_then(|cap| cap.get(1).map(|m| m.as_str()))
        {
            if let Some(decoded) =
                decode_base64_any(encoded).and_then(|bytes| String::from_utf8(bytes).ok())
            {
                out.push(decoded);
            }
        }
        for pattern in [r#"go_to_playerVast\('(.+?)'"#, r#"go_to_player\('(.+?)'"#] {
            if let Some(link) = Regex::new(pattern)
                .unwrap()
                .captures(onclick)
                .and_then(|cap| cap.get(1).map(|m| m.as_str().to_string()))
            {
...
fn sort_streams(streams: &mut [VideoStream], request: &Value) {
    ...
    streams.sort_by_key(|stream| {
        ...
        (
            ...
            value.contains(quality.trim_end_matches('p')),
            quality_rank(&value),
        )
    });
    streams.reverse();
...
fn filter_path(request: &Value) -> Option<String> {
    if let Some(genre) = filter(request, "genre").filter(|value| !value.is_empty()) {
        return Some(match genre.as_str() {
            "animes" => "/genres_animes".to_string(),
            "peliculas" => "/genres".to_string(),
            "series" => "/genres_series".to_string(),
            "tendencias" | "ratings" | "genre_series/toons" => format!("/{genre}"),
            _ => format!("/genres/{genre}"),
        });
    }
    if let Some(platform) = filter(request, "platform").filter(|value| !value.is_empty()) {
        return Some(format!("/network/{platform}"));
    }
    if let Some(year) = filter(request, "year").filter(|value| !value.is_empty()) {
        return Some(format!("/year/{year}"));
    }
    let type_filter = filter(request, "type").unwrap_or_else(|| "tendencias".to_string());
    Some(match type_filter.as_str() {
        "serie" => "/series".to_string(),
        "pelicula" => "/peliculas".to_string(),
        "anime" => "/animes".to_string(),
        "toon" => "/genre_series/toons".to_string(),
        "todos" => "/".to_string(),
        _ => "/tendencias".to_string(),
    })
}

fn iframe_src(body: &str) -> Option<String> {
...
const LIST_FIXTURE: &str = r#"<article class="item"><a href="/pelicula/sample"><img alt="Sample" data-srcset="/sample.jpg"></a></article>"#;
const DETAILS_FIXTURE: &str = r#"<div class="sheader"><div class="poster"><img src="/sample.jpg"></div><div class="data"><h1>Sample</h1><div class="sgeneros"><a>Drama</a></div></div></div><div id="single"><div class="wp-content">Sample synopsis.</div></div>"#;
const WATCH_FIXTURE: &str =
    r#"<li class="dooplay_player_option" data-type="movie" data-post="1" data-nume="1"></li>"#;
