...
const SOURCE: EstrenosDoramas = EstrenosDoramas;
const BASE_URL: &str = "https://estrenosdoramas.es";

struct EstrenosDoramas;

impl VideoSource for EstrenosDoramas {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = format!("{BASE_URL}/temporadas/?page={page}&order={order}");
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if !query.is_empty() {
            ...
            let qs = filter_params(
                &request,
                [
                    "genre", "season", "studio", "country", "network", "status", "type", "order",
                ],
            );
            if qs.is_empty() {
                format!("{BASE_URL}/temporadas/?page={page}&order=popular")
            } else {
                format!("{BASE_URL}/temporadas/?{qs}&page={page}")
            }
        };
        Ok(parse_cards(&fetch(&target, LIST_FIXTURE, BASE_URL)))
    }
    ...
    fn episodes(&self, request: Value) -> ExtensionResult<Vec<VideoEpisode>> {
        ...
        Ok(parse_episodes(&fetch(&referer, DETAILS_FIXTURE, BASE_URL)))
    }
    ...
    fn streams(&self, request: Value) -> ExtensionResult<Vec<VideoStream>> {
        ...
        for el in doc.select(&selector("[data-embed]")) {
            let link = attr(&el, "data-embed");
            let embed = redirected_embed(&link, &referer).unwrap_or(link);
            if !embed.is_empty() {
...
fn parse_cards(body: &str) -> Paged<CatalogItem> {
    let doc = Html::parse_document(body);
    let entries = doc
        .select(&selector(".listupd article a"))
        .filter_map(card)
        .collect();
    Paged {
        entries,
        has_next_page: body.contains("hpage")
...
fn card(el: ElementRef<'_>) -> Option<CatalogItem> {
    ...
    Some(CatalogItem {
        ...
        title: {
            ...
            if title.is_empty() {
                select_attr(el, "img", "alt").unwrap_or_else(|| title_from_path(&href))
            } else {
                title
            }
        },
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&doc, ".entry-title, h1").unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&doc, ".thumb img, img", "data-src")
            .or_else(|| select_attr_doc(&doc, ".thumb img, img", "src"))
            .map(|src| absolute_url(&src)),
        url: Some(absolute_url(path)),
        description: select_text_doc(&doc, ".mindesc"),
        tags: select_texts_doc(&doc, ".genxed a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str) -> Vec<VideoEpisode> {
    let doc = Html::parse_document(body);
    doc.select(&selector("#myList li a"))
        .enumerate()
        .map(|(idx, el)| {
            let href = attr(&el, "href");
            let title =
                select_text(el, ".epl-title").unwrap_or_else(|| format!("Episodio {}", idx + 1));
...
fn redirected_embed(link: &str, referer: &str) -> Option<String> {
    ...
    Regex::new(r#"https?://[^\s"'<>\\]+"#)
        .ok()?
        .captures_iter(&body)
        .filter_map(|c| c.get(0).map(|m| m.as_str().trim_matches('"').to_string()))
        .find(|u| !u.contains(BASE_URL))
}
...
const DETAILS_FIXTURE: &str = r#"<h1 class="entry-title">Sample</h1><ul id="myList"><li><a href="/sample-episode"><span class="epl-title">1</span></a></li></ul>"#;
const WATCH_FIXTURE: &str = r#"<div data-embed="https://example.invalid/embed"></div>"#;
