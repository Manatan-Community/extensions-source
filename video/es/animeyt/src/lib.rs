...
const SOURCE: Animeyt = Animeyt;
const BASE_URL: &str = "https://ytanime.tv";

struct Animeyt;

impl VideoSource for Animeyt {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let page = page(&request);
        let path = if listing(&request) == "latest" { "ultimos-animes" } else { "mas-populares" };
    ...
    fn search(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        ...
        let target = if !query.is_empty() {
            ...
            if params.is_empty() {
                format!("{BASE_URL}/mas-populares?page={page}")
            } else {
                format!("{BASE_URL}/mas-populares?{params}&page={page}")
            }
        };
    ...
    fn home(&self, request: Value) -> ExtensionResult<Vec<HomeSection<CatalogItem>>> {
        ...
        Ok(vec![
            ...
            HomeSection {
                id: "latest".to_string(),
                title: "Ultimos animes".to_string(),
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    ...
    let entries = select_all(&document, "div.video-card")
        .into_iter()
        .filter_map(card_from_video_card)
        .collect();
    Paged {
        entries,
...
fn card_from_video_card(el: ElementRef<'_>) -> Option<CatalogItem> {
    let href = select_attr(el, "div.video-title a, a[href]", "href")?;
    if href.is_empty() {
        return None;
    }
    Some(CatalogItem {
        key: path_key(&href),
        title: select_text(el, "div.video-title a")
            .or_else(|| select_attr(el, "img", "alt"))
            .unwrap_or_else(|| title_from_path(&href)),
        cover: select_attr(el, "div.video-card-image img, img", "src")
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&document, "#info .sa-title-series__title span, h1")
            .unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&document, "div.sa-series-dashboard__poster img, img", "src")
            .map(|src| absolute_url(&src)),
        url: Some(absolute_url(path)),
        description: select_text_doc(&document, "#info p.sa-text, .sa-text"),
        tags: select_texts_doc(&document, "nav.Nvgnrs a, .generos a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str) -> Vec<VideoEpisode> {
    let document = Html::parse_document(body);
    let mut episodes = select_all(&document, "#caps ul.list-group li.list-group-item a, a[href*='/ver/']")
        .into_iter()
        .filter_map(|el| {
            ...
            let title = select_text(el, "span.sa-series-link__number, .font-semibold")
