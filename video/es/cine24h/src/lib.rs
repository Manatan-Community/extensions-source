...
const SOURCE: Cine24h = Cine24h;
const BASE_URL: &str = "https://cine24h.online";

struct Cine24h;

impl VideoSource for Cine24h {
    fn list(&self, request: Value) -> ExtensionResult<Paged<CatalogItem>> {
        let page = page(&request);
        let target = if listing(&request) == "latest" {
            format!("{BASE_URL}/page/{page}/?s=trfilter&trfilter=1")
        } else {
            format!("{BASE_URL}/peliculas-mas-vistas/page/{page}")
        };
...
fn parse_cards(body: &str) -> Paged<CatalogItem> {
    ...
    let entries = doc
        .select(&selector(".TPost a:not([target])"))
        .filter_map(card_from_anchor)
        .collect();
    Paged {
        entries,
        has_next_page: body.contains("wp-pagenavi") && body.contains("next"),
    }
}
fn card_from_anchor(el: ElementRef<'_>) -> Option<CatalogItem> {
    let href = attr(&el, "href");
    let title = select_text(el, ".Title").or_else(|| select_attr(el, "img", "alt"))?;
    ...
    Some(CatalogItem {
        ...
        cover: select_attr(el, ".Image img, img", "data-src")
            .or_else(|| select_attr(el, ".Image img, img", "src"))
            .map(|src| absolute_url(&src).replace("/w185/", "/w300/")),
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&doc, "h1, .Single .Title").unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&doc, ".Single .Image img, img", "data-src")
            .or_else(|| select_attr_doc(&doc, ".Single .Image img, img", "src"))
            .map(|src| absolute_url(&src).replace("/w185/", "/w500/")),
        url: Some(absolute_url(path)),
        description: select_text_doc(&doc, ".Single .Description"),
        tags: select_texts_doc(&doc, ".Single .InfoList a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
        status: if path.contains("/movie/") {
...
fn parse_episodes(body: &str, item_path: &str) -> Vec<VideoEpisode> {
    ...
    let mut idx = 1.0;
    let mut out = Vec::new();
    for row in doc.select(&selector(".AABox .TPTblCn tr")) {
        let href = select_attr(row, ".MvTbTtl a", "href").unwrap_or_default();
        if href.is_empty() {
            continue;
        }
        let season = row
            ...
            .find_map(|e| select_text(e, ".Title"))
            .unwrap_or_default()
            .replace("Temporada", "")
            .trim()
            .to_string();
        let title = select_text(row, ".MvTbTtl a").unwrap_or_else(|| format!("Episodio {idx}"));
        let ep = select_text(row, ".Num").unwrap_or_default();
        out.push(VideoEpisode {
            key: path_key(&href),
            title: Some(format!("T{season} - E{ep} - {title}")),
...
fn player_embeds(body: &str, referer: &str) -> Vec<String> {
    let re = Regex::new(r#"src=["']([^"']+)["']"#).unwrap();
    let mut out = Vec::new();
    for block in body.split("TPlayerTb").skip(1) {
        ...
        {
            let nested = fetch(&src, "", referer);
            if let Some(inner) = re
                .captures(&nested)
                .and_then(|c| c.get(1))
                .map(|m| absolute_remote(m.as_str(), &src))
            {
                out.push(inner);
            } else {
                out.push(src);
...
const LIST_FIXTURE: &str = r#"<article class="TPost"><a href="/movie/sample"><span class="Title">Sample</span><div class="Image"><img src="/sample.jpg"></div></a></article>"#;
const DETAILS_FIXTURE: &str = r#"<h1>Sample</h1><div class="Single"><div class="Description">Sample description.</div><div class="Image"><img src="/sample.jpg"></div></div>"#;
const WATCH_FIXTURE: &str =
    r#"<div class="TPlayerTb"><iframe src="https://example.invalid/embed"></iframe></div>"#;
