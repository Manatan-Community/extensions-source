...
const SOURCE: AnimeMovil = AnimeMovil;
const BASE_URL: &str = "https://animemeow.xyz";

struct AnimeMovil;

impl VideoSource for AnimeMovil {
...
fn parse_listing(body: &str) -> Paged<CatalogItem> {
    ...
    let entries = select_all(&document, ".grid-animes li article a")
        .into_iter()
        .filter_map(card_from_link)
        .collect();
    Paged {
        entries,
...
fn card_from_link(el: ElementRef<'_>) -> Option<CatalogItem> {
    ...
    Some(CatalogItem {
        key: path_key(&href),
        title: select_text(el, "p:not(.gray)")
            .or_else(|| select_text(el, "p"))
            .unwrap_or_else(|| title_from_path(&href)),
        cover: select_attr(el, ".main-img img, img", "data-src")
            .or_else(|| select_attr(el, ".main-img img, img", "src"))
...
fn fetch_details(path: &str) -> CatalogItem {
    ...
    CatalogItem {
        key: path_key(path),
        title: select_text_doc(&document, ".banner-info div.titles h1, h1.text-4xl, h1")
            .unwrap_or_else(|| title_from_path(path)),
        cover: select_attr_doc(&document, "#anime_image", "data-src")
            .or_else(|| select_attr_doc(&document, "#anime_image", "src"))
            .or_else(|| select_attr_doc(&document, ".banner-img img, .main-img img", "src"))
            .map(|src| absolute_url(&src)),
        url: Some(absolute_url(path)),
        description: select_text_doc(&document, "#sinopsis, .mb-6 p.text-gray-300, .mb-6 p"),
        tags: select_texts_doc(&document, ".generos-wrap .item, .flex-wrap a"),
        language: Some("es".to_string()),
        content_rating: Some("safe".to_string()),
...
fn parse_episodes(body: &str) -> Vec<VideoEpisode> {
    let document = Html::parse_document(body);
    let mut episodes = select_all(&document, "#eps li > a, .divide-y li > a, a[href*='/ver/']")
        .into_iter()
        .filter_map(|el| {
            let href = attr(&el, "href");
            ...
            let title = select_text(el, ".font-semibold, p")
...
fn parse_hosters(body: &str, episode_url: &str) -> Vec<VideoHoster> {
    let mut out = Vec::new();
    let document = Html::parse_document(body);
    for el in select_all(&document, "#fuentes button[data-url]") {
        let mut embed = attr(&el, "data-url");
